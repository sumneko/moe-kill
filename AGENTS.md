...
- **不要过度防御**（用户 2026-09-19 定）：**正常流程下不会出现的情况不写防御**。调用约定与类型契约已经保证的东西（`ctx.random`、`ctx.desk`、自己模块内互相调用的参数、自己创建的对象）直接用 —— 我们相信 `ctx` 下有 `random` 且是我们定义的对象，不去管「万一被人篡改」；只有**运行期真的会缺**的（清单里没有内容包 ⇒ 没牌表、人数不在身份配置表里）与**外部输入**（协议 / 前端数据、别人放的包与文件）才检查。详见 `moe-kill-dev` 的 `references/code-style.md` 第 9 节。
