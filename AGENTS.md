...
- **注释尽量不写；接口留一行中文说明**（用户 2026-09-19 定）：代码里**不写注释**（说不清就先改名 / 拆函数）；但**入口**（函数 / 方法 / 字段 / 模块门面）保留**一行极短的中文说明**，几个字即可（如「获取标题」），贴在类型注解的 `#` 后。**存量注释待下次顺手清**。详见 `moe-kill-dev` 的 `references/code-style.md` 第 3 节。
