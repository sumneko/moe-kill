...
- **规则数值里的 `体力上限` 是「无武将时的默认上限」**（`package/@基础/配置.lua` 里为 4）：它是「体力初值 = 上限」与「主公 +1」两条规则的**基数**，所以现在必须留着（用户 2026-09-19 确认）；等**武将**接管上限后，它就只剩“武将没写的兵兜底”一个用途，可以退休。
...
- **落定机制（已实现）**：规则集用 `rule:on(时机名, function (ctx) ... end)` 注册、装配 / 流程代码用 `rule:fire(时机名, ctx)` 触发（时机名**不预设**，注册顺序即执行顺序，注册随清空重载清空）；每个时机的 `ctx` 类型写在 `server/rule/env-meta.lua`（纯类型文件，按事件名收窄字段类型；详见 `moe-kill-dev` 的 `references/architecture.md` 第 10 节）。
...
- **默认加载标记**：包目录名以 `@` 开头（`package/@基础`）⇒ 该包自动加载（清单不用写它，且排在最前），引用时**一律写不带 `@` 的名字**：`rule.depends { '../基础' }`、限定名 `基础.杀`、`getPackageMeta('基础')`。
