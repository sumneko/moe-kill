...
| 包 | 文件 |
| --- | --- |
| `@基础` | 配置 / 体力 / 攻击范围 / 牌堆（**每人建手牌 / 装备 / 判定三个区，手牌标成暗区**）/ 使用 / 抽牌 / 伤害 / 回复 / 濒死 / 判定 / 回合 / **收尾** / 基本牌（模板）/ **锦囊牌（模板）** / `meta.lua`（**`打出.lua` 已删** —— 打出的牌改由 `AskPlayCard:onAnswered` 交出） |
...
- 局上的入口：`game:canUse`（**用牌校验**：内建条目 + `'卡牌-能否使用'` 内容侧条目）/ `useCard` / `askCard` / **`askUseCard`** / **`askPlayCard`** / `ask` / `moveCard` / `damage` / `heal` / `draw` / `judge` / `createCard` / `createZone` / `enterDying` / `getDying` / `endGame` / `runFlow` / `registerFlow`
