...
> 验收口径：`server/bin/moe-kill.exe --test` ⇒ **479 用例 0 失败**；问题面板 information 及以上 **0**。
...
| 包 | 文件 |
| --- | --- |
| `@基础` | 配置 / 体力 / 攻击范围 / 牌堆（**每人建手牌 / 装备 / 判定三个区，手牌标成暗区**）/ 使用 / 打出 / 抽牌 / 伤害 / 回复 / 濒死 / 判定 / 回合 / **收尾** / 基本牌（模板）/ **锦囊牌（模板）** / `meta.lua` |
...
- **询问带合法选项**（2026-09-22；2026-09-23 加了 `zones`）：`game:askCard(被问者, 缘由, 条件?)` —— 条件是 `AskCard.Condition`（`name?` 牌名 + `targets?` 目标窗口 + `cards?` 一批牌 + **`zones?` 几个区整体**）；**内核在询问前遍历被问者的牌区、按条件算出 `ask.options`**（条件里给了 `targets` 就跑 `canUse`，于是内容侧的 `'卡牌-能否使用'` 条目一并生效；`targets = {}` 空表 = 只要求「至少有一个合法目标」；省略 `targets` = 不要求目标、不跑 `canUse`）；**答复必须落在选项里**（不在就拒收：`.err` = 原因、`.card` 不存在）；不给条件 = 不做限制。业务层三处调用点都是一行（`'出牌'` / `'打出'` / 求桃）
...
- **牌区可见性与归属**（2026-09-23，本批）：`Zone` 加了 `visible`（默认 `true` = 对所有人可见）与 `owner`（**`player:addZone` 自动记**；公共区没有），于是 `zone:setVisible(false)` = 「只有持有者看得见」（无归属的暗区 → 谁都不见），`zone:isVisibleTo(视角)` 给出结论；玩家侧区域从 1 个变 3 个：`手牌`（暗）/ `装备` / `判定`（**只建区，装备机制与判定阶段都还没做**）。`AskCard.Condition` 的 **`zones`** 与 `cards` 可以**同时给** ⇒ 一次询问就说完了「选一张你看得见的牌，或者盲选某个看不见的区」；【过河拆桥】/【顺手牵羊】就是这么写的（官方：暗区只能随机一张），`'获取目标'` 则要求「区域里有牌」
