...
- **三个通用的口子**（2026-09-23，为【五谷丰登】而开，但都不是它专用）：① **牌自己的两个钩子** `'结算前'` / `'结算后'`（各跑一次，ctx = 这次用牌的效果实例；夹在全局 `'卡牌-结算前'` / `'卡牌-结算后'` 内侧）—— 对应官方的「使用结算开始时 / 结束后」，将来的技能与无懈可击同用这两个点；② **`AskCard.Condition.cards`**：候选可以来自**给定的一批牌**（不再遍历被问者的牌区）—— 选“不属于任何人的牌”与将来选“别人区域里的牌”都靠它；③ **`Effect` 标签袋**（`setTag` / `getTag` / `removeTag`，与 `Phase` / `Player` 同形）：内容侧在一次结算里挂临时数据 —— 但**别拿它当牌账本**（会与真实位置漂移，见下一条）
- **临时处理区与效果收尾**（2026-09-23，本批）：`Effect` 自带一块**临时处理区**（`getTempZone()` 按需 `moe.zone.create()`；字段 `tempZone` 直接读 = 没建过就是空；**不进公共区表、没有区名**）+ **收尾时机 `'效果-收尾'`**（结完时触发一次 —— 正常 / `reject` / 取消 / 超时都算，ctx = 效果自己，在所有子效果结完之后、先于叫醒 `await` 的人；**内核只发信号、不挪牌**）。`package/@基础/收尾.lua`（新）订阅它把临时区里剩下的牌送 `弃牌`；【五谷丰登】改用它（亮出的牌进临时区 ⇒「剩余」= `tempZone:list()`，`'结算后'` 钩子与 `table.without` 都删了）。`Effect` 类从 `effect/init.lua` 搬到 `effect/effect.lua`，`init.lua` 只做装载（`core/init.lua` 里那 10 行缩成 1 行）。**待办（批二）**：判定 / 使用 / 打出 迁到临时区、`处理` 公共区的去留
