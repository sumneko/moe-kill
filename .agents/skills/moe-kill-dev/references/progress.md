...
- 应答方给一张**用不了的牌**（或给了牌没给目标）时：`UseCard:settle()` 的校验（牌不在手 / 没牌名 / 没定义 / `'获取目标'` 返回空 / 目标是空表 / 目标不合法）**全走 `error`** ⇒ 收进 `.err`、**不抛给调用方**；而失败发生在 `zone:take()` **之前**、牌没动 ⇒ 手牌没变 ⇒ 应答方给同样的答复就 `while true` **原地死循环**。
  最短复现：让应答方每次都给一张【闪】（没声明 `'获取目标'`）。
