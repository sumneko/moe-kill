...
| **效果分「主要 / 次要」** | 内核小重构（用户 2026-09-23 提）：只有主要效果才有处理区，「回溯查找」也只找最近的主要效果 —— 已定的两条与待定的两条见 §3 |
...
- **效果分「主要 / 次要」（用户 2026-09-23 提，尚未开工）**：起因 = 用户问「是不是得区分一下主要效果和次要效果，只有主要效果才有处理区；而且以后回溯查找的时候也可以只找最近的主要效果」。
  - **已核对的事实**：处理区（`getTempZone()`）现在**人人可建**，但实际只有 3 处真的建过区 —— `UseCard`（用掉的牌 / 【五谷丰登】亮出的牌）、`Judge`（判定牌 / 改判换下的牌）、`AskPlayCard` **借父的区**（打出的【闪】/【杀】，父实际是 `CardEffect`）；`CardEffect` / `AskCard` 一族 / `Ask` / `MoveCard` / `Draw` **从没人问它们要区**。`parent` 的**唯一**消费者就是 `AskPlayCard:onAnswered`（`package/` 一次都没读过它）。`'效果-收尾'` 现在**每个效果**都发一次（含次要的，只是没人订阅）。
  - **用户已定**：① 判据 = **类上标记**（`Effect` 基类默认**主要**），只把**询问三兄弟（`AskCard` / `AskUseCard` / `AskPlayCard`）+ `Ask` + `MoveCard`** 标成次要 —— 标少数派更省，漏标无害（过程性效果多了个区而已）；② **严格** —— 次要效果**不发** `'效果-收尾'`、调 `getTempZone()` **报错** ⇒ 次要效果要用处理区就借**最近的主要效果**（`AskPlayCard` 改成从 `parent` 起往上找，没有主要祖先就不动那张牌）。
  - **待定**：① `CardEffect`（对某个目标的生效）算主要吗？用户说「看看官方口径」—— 我按已记录文字推的是**主要**（处置单位 = 「某一次结算结束 / 一次响应结束」，而【杀】是「按行动顺序对每个目标依次结算」⇒ 打出的【闪】属于该目标那次结算），**但没抓到规则集原文**（见 `sanguosha-rules` §10 新增的待确认条）。② 记牌器现在记的是**根效果**（`parent == nil`，与「主要效果」不是同一个集合 —— 装配侧在顶层直接 `game:askCard` 时那个**询问**会被当成一条结算记进去），要不要跟着改成「只记主要效果」也未定。
  - **影响面**：`Effect` 基类加标记 + 一个「往上找最近的主要效果」的读法 → `AskPlayCard` 改一行 → `env-meta.lua`（若改收尾触发条件）→ 文档 + 用例（`AskPlayCard` 现在断言「进父结算的临时区」）。
