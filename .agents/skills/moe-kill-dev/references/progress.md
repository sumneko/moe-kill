...
> 验收口径：`server/bin/moe-kill.exe --test` ⇒ **448 用例 0 失败**；问题面板 information 及以上 **0**。
...
- **行动顺序的迭代器**（2026-09-23）：`Desk:actionOrder(允许列表?, 起点?)` —— 每步现推下一个角色（不再预排一份名单）、按角色去重、轮回到起点就结束；起点省略 = **当前回合角色**（没有回合角色就报错，回合外要起点得显式给）。`use-card` 换用它（起点仍是使用者），`@基础/濒死.lua` 求桃那一圈改用它同款口径（从当前回合角色起，官方口径）
