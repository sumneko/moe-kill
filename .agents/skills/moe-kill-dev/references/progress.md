...
> 验收口径：`server/bin/moe-kill.exe --test` ⇒ **443 用例 0 失败**；问题面板 information 及以上 **0**。
...
| `标准` | 牌表（**逐张 102 张**，每张带花色与点数；**草稿待核对**）/ 卡牌：杀 / 闪 / 桃 / **无中生有（第一张锦囊）** / `meta.lua` |
...
| **其它普通锦囊 + 无懈可击** | 用牌链已经跑通（【无中生有】已落地）；无懈的插入点是基础规则在 `'卡牌-结算前'` 按 `isKind('锦囊')` 判断再问一圈，缺的是嵌套询问（`askSkill`） |
