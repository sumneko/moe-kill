...
> 验收口径：`server/bin/moe-kill.exe --test` ⇒ **463 用例 0 失败**；问题面板 information 及以上 **0**。
...
- **三个通用的口子**（2026-09-23，为【五谷丰登】而开，但都不是它专用）：① **牌自己的两个钩子** `'结算前'` / `'结算后'`（各跑一次，ctx = 这次用牌的效果实例；夹在全局 `'卡牌-结算前'` / `'卡牌-结算后'` 内侧）—— 对应官方的「使用结算开始时 / 结束后」，将来的技能与无懈可击同用这两个点；② **`AskCard.Condition.cards`**：候选可以来自**给定的一批牌**（不再遍历被问者的牌区）—— 选“不属于任何人的牌”与将来选“别人区域里的牌”都靠它；③ **`Effect` 标签袋**（`setTag` / `getTag` / `removeTag`，与 `Phase` / `Player` 同形）：内容侧在一次结算里挂临时数据（五谷丰登的“剩余”就是挂在这次用牌上、`'生效'` 里用 `ctx.parent` 取回）
