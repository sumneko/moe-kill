...
- **临时处理区与效果收尾**（2026-09-23，本批）：`Effect` 自带一块**临时处理区**（`getTempZone()` 按需 `moe.zone.create()`；字段 `tempZone` 直接读 = 没建过就是空；**不进公共区表、没有区名**）+ **收尾时机 `'效果-收尾'`**（结完时触发一次 —— 正常 / `reject` / 取消 / 超时都算，ctx = 效果自己，在所有子效果结完之后、先于叫醒 `await` 的人；**内核只发信号、不挪牌**）。`package/@基础/收尾.lua`（新）订阅它把临时区里剩下的牌送 `弃牌`；【五谷丰登】改用它（亮出的牌进临时区 ⇒「剩余」= `tempZone:list()`，`'结算后'` 钩子与 `table.without` 都删了）。`Effect` 类从 `effect/init.lua` 搬到 `effect/effect.lua`，`init.lua` 只做装载（`core/init.lua` 里那 10 行缩成 1 行）。**批二已完成**：判定 / 使用 / 打出的牌也进临时区，`处理` 公共区废弃（`@基础/牌堆.lua` 不再建它）
