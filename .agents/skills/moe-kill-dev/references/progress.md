...
> 验收口径：`server/bin/moe-kill.exe --test` ⇒ **386 用例 0 失败**；问题面板 information 及以上 **0**。
...
| 包 | 文件 |
| --- | --- |
| `@基础` | 配置 / 体力 / 攻击范围 / 牌堆 / 使用 / 打出 / 抽牌 / 伤害 / 回复 / 濒死 / 回合 / 使用限制 / `meta.lua` |
...
上一批「出牌阶段的使用限制」已做完（见 `openspec/changes/archive/` 里的 `add-usability-check`）。下面这些是用户已表态、还没开工的方向，**按一个功能点一批推进**（用户 2026-09-19 定），下一批做哪个由用户定：
...
（死亡奖惩 / 判定机制 / 即时锦囊 + 无懈可击 / 装备与距离 / 武将技能 / 会话与协议 —— 已上提到 §2 的候选表。）
