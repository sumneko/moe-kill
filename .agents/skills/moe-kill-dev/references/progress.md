...
> 验收口径：`server/bin/moe-kill.exe --test` ⇒ **455 用例 0 失败**；问题面板 information 及以上 **0**。
...
| 包 | 文件 |
| --- | --- |
| `@基础` | 配置 / 体力 / 攻击范围 / 牌堆 / 使用 / 打出 / 抽牌 / 伤害 / 回复 / 濒死 / 判定 / 回合 / 基本牌（模板）/ **锦囊牌（模板）** / `meta.lua` |
| `身份场` | 配置 / 开局 / 奖惩 / 胜负 / `meta.lua` |
| `标准` | 牌表（**逐张 102 张**，每张带花色与点数；**草稿待核对**）/ 卡牌：杀 / 闪 / 桃 / **无中生有 · 南蛮入侵 · 万箭齐发 · 桃园结义 · 决斗（五张普通锦囊）** / `meta.lua` |
...
- **判定动作（2026-09-23）**：内核 `Judge : Effect`（`kind` = `judge`）+ `game:judge(玩家, 缘由?)`（当场结算，结果读 `.card`）；三个时机 **`'判定-亮牌'` →（改判窗口）`'判定-前'` → `'判定-后'`**，`judge:replace(新牌)` **只在窗口里有效**（窗口外直接报错），被换下的记进 `judge.replaced`；**内核不搬牌、不认识牌面** —— 翻牌与收牌写在 `@基础/判定.lua`（判定牌在亮出到收尾之间不属于任何区，结果就是那张牌）；**取顶 / 洗回提炼成牌区接口**：`OrderedZone:draw(n)`（不够就少给）+ `setShortageHandler`（`@基础/牌堆.lua` 挂「弃牌全部洗回」），摸牌改用它 ⇒ `@基础/抽牌.lua` 里的 `recycleDiscard` 退休；**还没做**：判定区与延时锦囊、判定阶段的结算、技能的改判实现
...
- 时机 24 个 —— 清单见 `references/architecture.md` 第 10 节与 `server/core/loader/env-meta.lua`
...
| **其它普通锦囊 + 无懈可击** | 用牌链已经跑通（已落地 5 张：无中生有 / 南蛮入侵 / 万箭齐发 / 桃园结义 / 决斗）；剩下【五谷丰登】（要牌定义的两个钩子 + 询问的“候选来自给定的一批牌”，用户 2026-09-23 定晚点做）、【过河拆桥】【顺手牵羊】（要“选**别人区域里**的牌”，牵扯信息可见性）、【借刀杀人】（要“逼别人使用【杀】”）、【无懈可击】（要嵌套询问 `askSkill`） |
