...
> 验收口径：`server/bin/moe-kill.exe --test` ⇒ **449 用例 0 失败**；问题面板 information 及以上 **0**。
...
- **行动顺序的迭代器与顺序锚点**（2026-09-23）：`Desk:actionOrder(允许列表?, 起点?)` —— 每步现推下一个角色（不再预排一份名单）、按角色去重、轮回到起点就结束；起点省略 = **顺序锚点**。锚点是 **`game.turnPlayer or game.lastTurnPlayer`**（回合内是当前回合角色；**回合结束后是上一个回合角色** —— 官方『一名角色的回合结束后……若该角色存活，从其开始』），两者都没有（还没人开过回合）才报错。`use-card` 换用它（起点仍是使用者），`@基础/濒死.lua` 求桃那一圈改用它同款口径（从锚点起，官方口径）
