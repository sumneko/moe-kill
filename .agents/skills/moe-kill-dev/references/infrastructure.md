...
| `utility.lua` | **新增 `m.isStrictArray(t)`**（用户 2026-09-23 加）：判断是不是**严格数组**（索引从 1 连续到底），**空表不算**；**新增 `m.toList(value)`**（本工程自有）：把「单个值 / 列表 / 空表 / nil」统一成列表 —— 列表原样返回（空表也是空列表）、单个值包成一张表、**给 nil 还是 nil**（可选参数用「不存在」表达「没给」）。当年各模块自己的 `toPlayerList` / `toTargetList` 就是为了这件事，现已删掉改用它 | 一个「单个还是列表」的归一写法不该在每处重写一遍；`toList` 的 nil 口径与原来那些局部函数**完全一致**（`useCard` / `askCard` 的「空表 = 没指定目标」不受影响） |
