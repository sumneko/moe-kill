...
| `game:canUse(使用者, 牌, 目标?)` | **这张牌此刻能不能用**（普通方法，**不是效果**：不结算、不进记牌器）：内建条目（有牌名 / 有同名内容定义 / **牌在使用者手上（声明了 `zone` 时还要在声明的那个牌区里）** / `'获取目标'` 给出非空合法目标 / 目标非空且是合法目标的子集 / **次数没用满**；**声明了 `noTarget()` 的牌**只留「有牌名 / 有定义 / 在手上 / 次数没用满」—— 前两条目标判据都跳过，但**给了目标就是不成立**（明确失败，不静默忽略））→ 再问内容侧 **`'卡牌-能否使用'`** 时机（**返回非 nil 值即否决，返回值就是原因**；`return false` = 无原因，内核折算成一句通用原因）；返回 `ok` / 原因 / 合法目标三个值（**无目标牌的第三个值是「不存在」**）。**次数的内建条目**（2026-09-22 落地）：**阶段存在且阶段角色就是使用者**时，`阶段:getUseCount(名字) < `内容定义的限额 + 阶段上的上限增减`` 不成立就**直接返回原因、不但不触发 `'卡牌-能否使用'`**（“不通过就不用问规则”）；阶段属于别人或不在任何阶段里 ⇒ 不判（阶段外不受限）。`目标` 省略 = 只判"此刻能不能用"。筛选一遍手牌就要调它十几次 ⇒ **它同步且只读**（内建条目与内容侧条目都不要 `await`、不要改状态；**按名字记账 / 改上限用阶段实例的两个接口**，别在这里改 —— 内核在真正用出去时自己记一次）。**`askUseCard` 的候选全靠它**（用不了的牌不进选项，业务层不自己拼候选） |
...
| `CardDef:noTarget()` / `getNoTarget()` | **这张牌不指定目标**（官方装备牌）：声明后 `canUse` 跳过两条目标判据、`useCard` 允许零目标（牌自己的 `'结算前'` / `'结算后'` 照旧各跑一次，逐目标循环空转）；**调用方给了目标就是不成立**；`AskUseCard` 对这种牌给的选项**不带 `targets`** |
...
| `game:setSlots(区名, 槽位名表)` / `getSlots(区名)` | **声明某个牌区的槽位**（2026-09-24 落地，与规则数值同层）：内容侧在**加载期**声明一次（`@基础/装备.lua` 里 `game:setSlots('装备', { '武器', '防具', '进攻马', '防御马' })`），内核**只存不解释**（不认识「武器」是什么）；同一个区名重复声明以后写的为准、随重装清空；`getSlots` 给按声明顺序的快照，**没声明过就是「不存在」**（区名参数 ⇒ 保留 `get` 前缀，见 `code-style.md` 第 11 节）。**它只是定义，不是每个玩家的值** ⇒ 不挂 `'游戏-开始'`；建玩家时把这份名单带进槽位区，所以**中途建的玩家也拿得到**（`Player:__init` 读的就是它） |
| `SlotZone`（类，`moe.slotZone.create(局?, 槽位名表?)`） | **按槽位寻址的牌区**（与 `OrderedZone` 同一个道理：**结构性能力用区子类表达**，`kind` = `slotZone`）：`slots`（这个区声明了哪些槽位，公开字段，按声明顺序）；`getSlot(槽位名) -> Card?`（**懒校验**：槽位记的那张牌已经不在本区就就地失效、返回「不存在」⇒ **拆牌 / 抢牌路径一行结构代码都不用改**）；`putInto(槽位名, 牌)`（把牌放进那个槽位 —— **牌不必先在本区**，内部「在别的区就 `move`、没有归属就 `put`」；**同槽已有牌 ⇒ 内核把它同步送进那个局的 `弃牌`**，走 `self.game:getZone('弃牌')`、**不经 `MoveCard` 效果**（官方「同一槽位的旧装备置入弃牌堆」固定、没有变体，也没有可取消性）；旧牌离开区 ⇒ 它挂的修正由 `Card:bindZoneGC` 自动撤（见 `Card:bindZoneGC` 行））；**槽位名没声明过 / 建区时没给局（要替换时）都明确报错**。**内核不认识「武器 / 防具 / 进攻马 / 防御马」这四个词** —— 槽位名由内容声明、分类匹配也由内容侧做（`@基础/卡牌/装备牌.lua` 拿分类去 `zone.slots` 里找自己那条）。`装备` 区由 `Player:__init` 建成它（`player:getZone('装备')` 的重载收窄成 `SlotZone`）；判定区将来用 `OrderedZone`（它要的是顺序，不是槽位） |
| `Attributes:addModifier(名字, 增量)` | **加一条修正，返回撤销函数**（2026-09-24 落地；`Attributes` 实例从 `player:getAttributes()` 拿）：`add` 是单向变化（扣体力那种），而「加一条加成」按项目约定必须给出 disposer —— 返回的闭包**精确减掉这次加的量**（幂等、与别的加成可叠加、不互相覆盖）。**撤销挂在谁身上由调用方决定**：装备把它 `card:bindZoneGC(...)` 挂在牌上（见 `@基础/装备.lua` 的 `equip`） |
...
| `Card:bindZoneGC(撤销函数)` | **把一件事挂在「这张牌在牌区里」这段上**（2026-09-24 落地）：牌上带一个**懒建的私有容器** `zoneGCHost`（第一次 `bindZoneGC` 才 `moe.gc.host()` 现建，没挂过的牌零成本），`Card:bindZone` 发现**区真的变了**（`self.zone ~= zone`）就 `Delete` 掉容器、置空 —— 容器里的东西（`GCNode`）随之执行撤销函数。`bindZone` 是 `Zone` 的 `put` / `take` / `move` / `clear` 的**唯一收口点** ⇒ **不管谁在什么时候挪牌，牌一离开那个区就自动撤销**（同槽替换、被拆、死亡弃牌、将来任何路径），一条都不用靠调用约定，也不需要给「牌离开区」加时机。**同区内部调序（`move` 到自己）不算离开**。装备的「攻击范围 / 距离修正」修正就是这么挂的（见 `@基础/装备.lua` 的 `equip`） |
...
- **基础牌区：内核认五个名字，其余仍归内容侧**（2026-09-24 落地，取代原来的「内核零区域名」口径）：`抽牌`（局上、有序）/ `弃牌`（局上）由 `Game:__init` 建，`手牌` / `装备` / `判定`（玩家身上）由 `Player:__init` 建 —— **包不得重建**（`createZone` / `addZone` 撞名报错），取用照旧 `game:getZone('弃牌')` / `player:getZone('手牌')`（`getZone` 上有重载，这五个名字取出来是非空类型；`装备` 还收窄成 `SlotZone`，**建它时带的槽位名来自局上那份声明**）；**可见性不由内核定**（建的区默认 `visible = true`，`手牌` 标暗仍写在 `@基础/牌堆.lua`）；**结算的默认收尾也收在内核**（效果结完把临时区剩下的牌送 `弃牌`）。切分轴是「**固定 / 可配置**」：这五个区与「处理区的牌最后回弃牌堆」没有任何变体 ⇒ 内核；其余一切（可见性、牌表、洗回策略、数值、洗牌、摸牌时机）⇒ 内容侧。
...
- **攻击范围与距离**（2026-09-24 接上修正）：属性 `攻击范围`（默认 1）与两条修正属性（`进攻修正` / `防御修正`，默认 0）都由**基础规则给每个玩家写入**；「够不够得着」由内容包拿**距离**与攻击范围组合得出 —— 内核不作任何具体判定（见 `package/标准/卡牌/杀.lua`：「除使用者以外、且在攻击范围内」，因此**打不到自己**）。
  - **修正值走属性、求值在内容侧**：武器的加成 / 坐骑的 ±1 由 `@基础/装备.lua` 的 `equip` 用 `addModifier` 写进属性，**撤销挂在牌上**（`card:bindZoneGC` ⇒ 牌一离开装备区就自动撤，拆牌 / 换装 / 死亡弃牌一条调用约定都不用加）；`desk:getDistance` 仍**只按座位算**（内核不认识「马」与「武器」），`@基础/距离.lua` 的 `distance(from, to)` 把它加上**自己的 `进攻修正`**（进攻马 -1）与**对方的 `防御修正`**（防御马 +1），结算后**最小 1** —— `杀.lua` / `顺手牵羊.lua` / `借刀杀人.lua` 都用它。
  - **装备牌的数据写在牌定义上**（`CardDef:value`，内核只存不解释）：武器写 `攻击范围` 的**增量**（麒麟弓 ⇒ `4`，因为默认 1）、坐骑写 `距离修正`（进攻马 `-1` / 防御马 `1`）；**槽位名不另写一份数据** —— 分类里带着它（`{ '装备', '坐骑', '进攻马' }`），装备牌的基类拿分类去 `装备区.slots` 里匹配。
