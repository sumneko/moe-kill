...
| `game:canUse(使用者, 牌, 目标?)` | **这张牌此刻能不能用**（普通方法，**不是效果**：不结算、不进记牌器）：内建条目（有牌名 / 有同名内容定义 / 牌在使用者手上 / `'获取目标'` 给出非空合法目标 / 目标非空且是合法目标的子集 / **次数没用满**）→ 再问内容侧 **`'卡牌-能否使用'`** 时机（**返回非 nil 值即否决，返回值就是原因**；`return false` = 无原因，内核折算成一句通用原因）；返回 `ok` / 原因 / 合法目标三个值。**次数的内建条目**（2026-09-22 落地）：**阶段存在且阶段角色就是使用者**时，`阶段:getUseCount(名字) < `内容定义的限额 + 阶段上的上限增减`` 不成立就**直接返回原因、不但不触发 `'卡牌-能否使用'`**（“不通过就不用问规则”）；阶段属于别人或不在任何阶段里 ⇒ 不判（阶段外不受限）。`目标` 省略 = 只判"此刻能不能用"。筛选一遍手牌就要调它十几次 ⇒ **它同步且只读**（内建条目与内容侧条目都不要 `await`、不要改状态；**按名字记账 / 改上限用阶段实例的两个接口**，别在这里改 —— 内核在真正用出去时自己记一次）。**询问的条件里带 `targets` 时也是它在跑**（`askCard` 内部筛选项，业务层不自己拼候选） |
