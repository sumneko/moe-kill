...
| 接口 | 说明 |
| ---- | ---- |
| `Effect`（类，门面无工厂） | **效果基类**：持局 + 种类标识 `kind` + 外层效果 `parent`；`apply()` = 记父（栈顶）→ 压入结算栈 → `settle()`（子类实现）→ 退栈 |
| `game:play(使用者, 牌, 目标们)` | **使用一张牌**（`UseCard`，`kind` = `useCard`）：校验（牌在手 / 目标是合法目标的非空子集）→ 取出牌 → 按声明顺序跑「使用」回调 → 触发收尾时机；等价于「造 `UseCard` + `:apply()`」 |
...
- **内容侧的挂法**：牌的定义上按内容侧事件名登记 —— `Card '杀' :on('获取目标', fn) :on('使用', fn)`；`'获取目标'` 返回**合法目标列表**（`Player[]`），机制拿它同时做两件事：校验调用方给的目标是它的**非空子集**、并将来的「可用目标列表」直接复用。**fail-closed**：没声明钩子 / 钩子没返回列表 / 列表为空 ⇒ 这张牌**现在不能用**（明确失败、牌留在原处）；同一张牌声明多个钩子时取**交集**（每个声明只能收窄）。回调的上下文就是**这次使用的效果实例**（`UseCard`，字段 `user` / `card` / `targets`）。钩子名固定（清单以 `env-meta.lua` 为准，拼错编辑期报错），**内核不为钩子名加专门方法**。
...
- **桌子与玩家的属性式读取**（用户 2026-09-19 定）：座位列表与“还活着的”列表直接读字段 —— `desk.players` / `desk.alivePlayers`（`Class` 的 `__getter` 实现，**每次现算不缓存**），不再有 `desk:getPlayers()` 这种 getter 方法。存活的判断是 `player:isAlive()`（默认活着）；**`player:setAlive(false)` 从活变死时触发 `'玩家-死亡'`**（上下文 = 这个玩家），重复置死不再触发、复活后可再触发。桌子与玩家在**局建立**（`desk:bindGame`）/ **入座**时获得 `game` 引用，因此玩家在没上桌时置存活状态也不报错（只是没局就不发时机）。
...
- **本批边界**（见 `openspec/changes/archive/2026-09-19-add-slash/proposal.md` 与 `2026-09-19-effect-stack/proposal.md`）：没有回合与阶段（因此不限阶段、不限次数）、没有「打出」与响应（闪）、**没有无懈可击与濒死插入**（结算栈已就位，插入逻辑未做）、伤害不处理濒死 / 死亡（体力可为负）、没有装备与判定。
- 测试：`--test core.play`（校验 / 取牌 / 结算回调 / 收尾时机 / 栈恢复 / 合法目标列表：没声明、空列表、非子集、空目标、多钩子交集）、`--test core.effect`（效果与结算栈：压退 / 嵌套 / 抛错也退 / 快照 / 撤销函数 / 深度上限）、`--test core.damage`（伤害实例 / 改体力 / 可为负 / 时机先后与同一个实例 / 结算期间在栈上）、`--test rule.slash`（「杀」端到端：攻击范围 / 造成伤害 / 进弃牌堆 / 不能对自己用）。
