...
| `game:canUse(使用者, 牌, 目标?)` | **这张牌此刻能不能用**（普通方法，**不是效果**：不结算、不进记牌器）：内建条目（有牌名 / 有同名内容定义 / 牌在使用者手上 / `'获取目标'` 给出非空合法目标 / 目标非空且是合法目标的子集）→ 再问内容侧 **`'卡牌-能否使用'`** 时机（**返回非 nil 值即否决，返回值就是原因**；`return false` = 无原因，内核折算成一句通用原因）；返回 `ok` / 原因 / 合法目标三个值。`目标` 省略 = 只判"此刻能不能用"。筛选一遍手牌就要调它十几次 ⇒ **它同步且只读**（校验条目里不要 `await`、不要改状态；要计数就挂 `'卡牌-结算前'`） |
...
| `game:askCard(被问者, 缘由, 合法选项?)` | **要一张牌**（`moe.askCard` 造 `AskCard : Effect`，`kind` = `askCard`）：返回**询问实例**（同样「驱动 + 等」），询问通过局上的 **`'卡牌-询问'`** 时机交给应答方，应答方调 `ask:answer{ card = 那张牌, targets = 目标 }`；**答复读 `.card` / `.targets`**（转存自 `.result`，后者是答复本体 `AskCard.Answer`），没人答上或被取消 ⇒ 「不存在」。**缘由**是不透明字符串（内核不解释、只原样带给规则层）；**合法选项**是第三参数（`AskCard.Option[]` = 牌 + 这张牌的可用目标，**发起方算好传进来、内核只持有**，应答方读 `ask.options`）—— **答复必须落在选项里**：牌不在选项里 / 选项带 `targets` 而答复没给目标、或给了选项外的目标 / 选项不带 `targets` 而答复给了目标 ⇒ **拒收**（`.err` = 原因、`.card` 为「不存在」，对调用方与"没答上"同一个分支）；**不给选项 = 不做限制**。**应答方给出了答复**时才触发 **`'卡牌-答复'`** 与紧接着的 **`'卡牌-答复后'`**（上下文 = 这次询问；没人应答不触发），规则层据此决定"这次答复意味着什么、那张牌怎么处置"（基础规则：缘由是「打出」⇒ 先进 `处理`、再进 `弃牌`） |
...
- **询问与应答方**：内容侧要一张牌就 `game:askCard(被问者, 缘由, 合法选项)` —— 它是一次效果（有父效果、可被取消、`kind` = `askCard`），**答复就是这次询问的结果**（读 `.card` / `.targets`，**应答一给出就定下** —— 答复时机里 `ctx.card` 就读得到）。**缘由**（第二个参数）是不透明字符串（如 `'打出'`）：内核**不解释**，只原样带给规则层。**合法选项**（第三个）是"这次可以给哪些牌"（`AskCard.Option[]`：牌 + 它的可用目标）：**发起方算好传进来，内核只持有并校验答复是否落在里面** —— 内核零语义（不解释牌名、不预设区名 ⇒ 它自己没法从手牌里挑；将来有装备区还会挑错，装备牌不能打出），但"答复必须在选项里"这道校验只能由内核做（协议 / AI 给的答复是**外部输入**）。于是**筛选是发起方的事、把关是内核的事**（一起做才能让前端拿到可选项又不至于乱答）。内核**不写死「谁来答」**：询问通过局上的 **`'卡牌-询问'`** 时机送出去，应答方拿到询问实例后调 **`ask:answer{ card = …, targets = … }`**（**`targets` 给单个或一张列表都行** —— 入库前统一成列表，所以 `.targets` 恒为 `Player[]`；答不上就 `answer(nil)` 或干脆不调）；应答方可以**让出**（当场拿不到就等会儿再答，业务侧照旧像写同步代码一样写）；**没答上不是失败**（`ask.card` 为「不存在」），只有**重复应答**才记一条 `info`（先答的算数）。这一套只服务「要一张牌」的问法 —— 要技能 / 数字的问法将来另开一个类（`askSkill`…）。
