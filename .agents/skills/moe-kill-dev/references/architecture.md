...
规则侧第一个功能点：让一张牌能「用出去并产生后果」。机制在内核、规则全在内容包 —— 规格见 `openspec/specs/core-play/spec.md` 与 `openspec/specs/core-damage/spec.md`。

| 接口 | 说明 |
| ---- | ---- |
| `game:play(使用者, 牌, 目标们)` | **使用一张牌**：在使用者名下牌区里定位该牌 → 按牌自己的声明校验目标合法性 → 取出 → 按声明顺序执行该牌的「使用」回调 → 触发收尾时机 |
| `moe.damage.create { game, from, to, amount }` | **造一次伤害**（`Damage` 实例）：字段 `game` / `from` / `to` / `amount` 可读 |
| `伤害:apply()` | **结算这次伤害**：触发 `'伤害-前'` → 目标的 `体力` 减去点数 → 触发 `'伤害-后'` |
...
- 测试：`--test core.play`（校验 / 取牌 / 结算回调 / 收尾时机）、`--test core.damage`（伤害实例 / 改体力 / 可为负 / 时机先后与同一个实例）、`--test rule.slash`（「杀」端到端：攻击范围 / 造成伤害 / 进弃牌堆）。
