...
- **载体一：局实例**（`game` 上的字段）—— 已建好的局跨重载照常可用、`__init` 也不会重跑，所以挂在局上的东西既不会丢也不会被换成新的。**发号器就是这么放的**：`game:nextId()`（局内递增、牌与将来的技能共用；`__init` 里给初值、不放进 `resetContent`）。属于某一局的数据一律用这个载体，不要往内核级状态上挂。
...
| `game:canUse(使用者, 牌, 目标?)` | **这张牌此刻能不能用**（普通方法，**不是效果**：不结算、不进记牌器）：内建条目（有牌名 / 有同名内容定义 / 牌在使用者手上 / `'获取目标'` 给出非空合法目标 / 目标非空且是合法目标的子集）→ 再问内容侧 **`'卡牌-能否使用'`** 时机（**返回非 nil 值即否决，返回值就是原因**；`return false` = 无原因，内核折算成一句通用原因）；返回 `ok` / 原因 / 合法目标三个值。`目标` 省略 = 只判"此刻能不能用"。筛选一遍手牌就要调它十几次 ⇒ **它同步且只读**（校验条目里不要 `await`、不要改状态；要计数就挂 `'卡牌-结算前'`）。**询问的条件里带 `targets` 时也是它在跑**（`askCard` 内部筛选项，业务层不自己拼候选） |
| `game:useCard(使用者, 牌, 目标)` | **使用一张牌**（`UseCard`，`kind` = `useCard`）：**先 `game:canUse` 校验** —— 不过就 `self:reject(原因)` ⇒ 这次生效以「不成立」收尾（`.err` = 原因、**不走错误处理器、不算故障**、**不取牌、不触发 `'卡牌-结算前'`**，牌留在原处）；过了才取出牌 → 触发 **`'卡牌-结算前'`**（内容侧据此安置它，基础规则放进 `处理`）→ **按行动顺序对每个目标各结算一次生效** → 触发 **`'卡牌-结算后'`**（基础规则据此送进 `弃牌`）；等价于「造 `UseCard` + `:apply()`」。**目标接受单个或列表**（入口归一，与 `moveCard` 同一写法） |
