...
| `game:useCard(使用者, 牌, 目标)` | **使用一张牌**（`UseCard`，`kind` = `useCard`）：校验（牌在手 / 目标是合法目标的非空子集）→ 取出牌 → 触发 **`'卡牌-结算前'`**（内容侧据此安置它，基础规则放进 `处理`）→ **按行动顺序对每个目标各结算一次生效** → 触发 **`'卡牌-结算后'`**（基础规则据此送进 `弃牌`）；等价于「造 `UseCard` + `:apply()`」。**目标接受单个或列表**（入口归一，与 `moveCard` 同一写法） |
| `CardEffect`（类，门面无工厂） | **对某个目标的一次生效**（`kind` = `cardEffect`）：字段 `def` / `user` / `card` / `target`，父效果 = 这次用牌；跑这张牌的 `'生效'` 回调 |
| `game:askCard(被问者, 缘由, 匹配条件)` | **要一张牌**（`moe.askCard` 造 `AskCard : Effect`，`kind` = `askCard`）：返回**询问实例**（同样「驱动 + 等」），询问通过局上的 **`'卡牌-询问'`** 时机交给应答方，应答方调 `ask:answer{ card = 那张牌, targets = 目标 }`；**答复读 `.card` / `.targets`**（转存自 `.result`，后者是答复本体 `AskCard.Answer`），没人答上或被取消 ⇒ 「不存在」。**缘由**是不透明字符串（内核不解释、只原样带给规则层）；**匹配条件**是第三参数（要什么样的牌，`{}` = 任意牌；**内核同样不解释、不校验**，候选由应答方按它自己算）；**应答方给出了答复**时才触发 **`'卡牌-答复'`** 与紧接着的 **`'卡牌-答复后'`**（上下文 = 这次询问；没人应答不触发），规则层据此决定"这次答复意味着什么、那张牌怎么处置"（基础规则：缘由是「打出」⇒ 先进 `处理`、再进 `弃牌`） |
...
| `game:countOperation()` / `game:resetOperations()` | **一回合的操作上限**（用户 2026-09-22 定）：内核**每起一次结算就记一次**（`Effect:apply()` 里调 `countOperation`），一个回合超过 `Game.MAX_OPERATIONS`（**1000**）就 **`error`**；`resetOperations()` 由**内容侧在回合开始时调**。用途：防内容写死循环（例：出牌阶段一直拿到用不了的牌）。**报错 = 故障信号**（真的转不完了），不是控制流 |
...
- **操作上限：防内容写死循环的兜底**（用户 2026-09-22 定）：内核自己管不了一个内容侧的 `while true`，但**凡是走内核的循环都会被计数** —— 每次起结算（`Effect:apply()`）就 `+1`，超过 1000 就报错。它买到的不是"正确性"而是**"卡死了有人喊"**：出牌阶段拿到用不了的牌时会以 `error` 收场、流程任务 `.err` 里能看到原因，而不是默默转到天荒地老（也不至于把事件循环饿死）。**计数与重置分家**：内核只管数，"什么时候算一个回合"由内容说（`@基础/回合.lua` 在 `runTurn` 开头调 `resetOperations()`）—— 内核仍然不认识"回合"。
...
- **询问与应答方**：内容侧要一张牌就 `game:askCard(被问者, 缘由, 匹配条件)` —— 它是一次效果（有父效果、可被取消、`kind` = `askCard`），**答复就是这次询问的结果**（读 `.card` / `.targets`，**应答一给出就定下** —— 答复时机里 `ctx.card` 就读得到）。**缘由**（第二个参数）是不透明字符串（如 `'打出'`）：内核**不解释**，只原样带给规则层。**匹配条件**（第三个）是"要什么样的牌"，`{}` = 任意牌；**内核也不解释、不校验**（候选由应答方算 —— 发起方不必把候选列表塞进询问）。内核**不写死「谁来答」**：询问通过局上的 **`'卡牌-询问'`** 时机送出去，应答方拿到询问实例后调 **`ask:answer{ card = …, targets = … }`**（**`targets` 给单个或一张列表都行** —— 入库前统一成列表，所以 `.targets` 恒为 `Player[]`；答不上就 `answer(nil)` 或干脆不调）；应答方可以**让出**（当场拿不到就等会儿再答，业务侧照旧像写同步代码一样写）；**没答上不是失败**（`ask.card` 为「不存在」），只有**重复应答**才记一条 `info`（先答的算数）。这一套只服务「要一张牌」的问法 —— 要技能 / 数字的问法将来另开一个类（`askSkill`…）。
