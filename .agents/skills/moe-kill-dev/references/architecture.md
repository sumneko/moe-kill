...
| `game:askUseCardToCard(被问者, 缘由, 条件?)` | **要一次「对一张牌的使用」**（`moe.askUseCardToCard` 造 `AskUseCardToCard : AskCard`，`kind` = `askUseCardToCard`）：与 `askUseCard` 同一套机制，两处不同 —— ① 条件多一条 **`target?`（目标牌）**，即 `{ name = '无懈可击', target = 那张牌 }`；② 候选**逐张跑 `game:canUseToCard`**，选项 = `{ card = 那张牌, target = 目标牌 }`，**答复只需给牌**（目标已经定了，不用也不允许再给目标）。【无懈可击】的窗口用它 |
