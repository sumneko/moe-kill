...
```
  ...
  类型名的**最后一段** = 事件名**去掉连字符**（连字符不是合法的 LuaDoc 类型名字符），前面挂在自己类的命名空间下（`Game.EventCtx.游戏开始`、`Game.EventCtx.卡牌`）；上下文的类型也可以直接就是内核的某个类（如「伤害-前」与「伤害-后」的上下文就是 `Damage` 实例）。同名多行 `---@field` 会被当成多个签名候选（实测不报 `duplicate-doc-field`），**字面量事件名走专用那条、动态名字落到 `string` 兜底那条** —— 最后两条兜底候选不能省，否则动态时机名会被误报类型不匹配。新增时机时**顺手补一条**：不补也能跑（注入面与运行时都不校验时机名），只是调用点拿不到 `ctx` 的字段类型。
...
- **内容侧的挂法**：牌的定义上按内容侧事件名登记 —— `Card '杀' :on('目标合法', fn) :on('使用', fn)`；`'目标合法'` 返回 `false` 表示不合法（其它返回值视为通过），不合法时使用失败且牌留在原处；回调的上下文就是**这次使用的效果实例**（`UseCard`，字段 `user` / `card` / `targets`）。
