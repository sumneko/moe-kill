...
Card '杀'
    ...
    : on('生效', function (ctx)
        ...
        if card then                                                    -- 这里约定答复就是把那张闪本身
            game:moveCard({ card }, '弃牌堆')                            -- 去向由内容侧处置（moveCard 待实现）
            return                                                      -- 有闪就不受伤
        end
...
- 测试：`--test core.play`（校验 / 取牌 / 逐目标生效与行动顺序 / 单个生效可被取消 / 收尾时机 / 栈恢复 / 合法目标列表）、`--test core.effect`（效果链：父子 / 嵌套 / 结果与失败 / 取消 / 递归上限）、`--test core.ask-card`（询问往返 / 被问者与答复与父效果 / 多问互不串 / 没人应答 / 重复应答被拒 / **应答方让出后再答** / 被取消的询问；打出的取出与收尾时机）、`--test core.damage`、`--test rule.slash`（「杀」端到端：攻击范围 / 造成伤害 / 进弃牌堆 / 不能对自己用 / 打出闪就不受伤 / 多目标依次）。
