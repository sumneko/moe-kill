...
| `game:ask(被问者, 问什么)` | **当场要一个决策**（`moe.ask` 造 `Ask : Effect`，`kind` = `ask`）：返回**询问实例**（同样「驱动 + 等」），答案读 `.result` —— 回答者返回什么就是什么；没答上（`nil` / `false`）或被取消 ⇒ 「不存在」；这一局没有回答者 ⇒ 失败 |
...
- **询问与回答者**：内容侧要一个决策就 `game:ask(被问者, 问什么)` —— 它是一次效果（有父效果、可被取消、`kind` = `ask`），**答案就是这次询问的结果**（`ask.result`），读法与 `game:damage` / `game:useCard` 一致。**回答者由局持有**（`game.answerer = function (ask) return 答案 end`），无头测试注入脚本化回答者、将来会话层换成向前端提问。本批是**当场取答案**：回答者同步返回什么就是什么（`nil` / `false` 也算「没答上」，`ask.result` 为「不存在」），**只有「这一局没有回答者」才是失败** —— 答案的形状（例如 `{ card = 打出的那张牌 }`）由内容侧约定，内核不解释。「稍后答复 + 超时」要等会话层能接住挂起。
...
Card '杀'
    ...
    : on('生效', function (ctx)
        ...
        if ask.result then                                -- 这里约定答案是 { card = 打出的那张牌 }
...
- 测试：`--test core.play`（校验 / 取牌 / 逐目标生效与行动顺序 / 单个生效可被取消 / 收尾时机 / 栈恢复 / 合法目标列表）、`--test core.effect`（效果链：父子 / 嵌套 / 结果与失败 / 取消 / 递归上限）、`--test core.ask`（询问往返 / 结果与父效果 / 多问互不串 / 无回答者时报错 / 拿不出答案与答「否」 / 被取消的询问；打出的取出与收尾时机）、`--test core.damage`、`--test rule.slash`（「杀」端到端：攻击范围 / 造成伤害 / 进弃牌堆 / 不能对自己用 / 打出闪就不受伤 / 多目标依次）。
