...
- **攻击范围与距离**（2026-09-24 接上修正）：属性 `攻击范围`（默认 1）与两条修正属性（`进攻修正` / `防御修正`，默认 0）都由**基础规则给每个玩家写入**；「够不够得着」由内容包拿**距离**与攻击范围组合得出 —— 内核不作任何具体判定（见 `package/标准/卡牌/杀.lua`：「除使用者以外、且在攻击范围内」，因此**打不到自己**）。
  ...
  - **装备牌的数据写在牌定义上**（`CardDef:value`，内核只存不解释）：武器写 `攻击范围` 的**增量**（麒麟弓 ⇒ `4`，因为默认 1 —— **逐张不同**，所以写在每张牌上）；坐骑的 `距离修正` **逐张都一样**（进攻马恒 `-1` / 防御马恒 `1`），所以写在**两个马定义**上（`进攻马` / `防御马`）—— 具体马只剩一条 `: extends '进攻马'`。**槽位名不另写一份数据** —— 分类里带着它（`进攻马` / `防御马` 两个定义各自 `addKind` 自己那条），`装备牌.lua` 拿分类去 `装备区.slots` 里找自己那条；两个马定义的**钩子是从 `坐骑牌` 抄来的**（`extends` 会带上基类的钩子），因为两条钩子逻辑一模一样（只是按 `slot` 对应到不同的修正属性），所以 `坐骑牌` 上的钩子直接 `card:getValue('距离修正')` 取那个值（值由马定义保证在，不用判空）。
