...
| `game:canUseToCard(使用者, 牌, 目标牌?)` | **这张牌能不能「对一张牌使用」**（普通方法，与 `canUse` 平行、**同步且只读**）：牌本体那半与 `canUse` **共用同一段内核校验**，差别只在合法性依据 —— 要求**定义声明过 `'对卡牌生效'`**（否则原因写成“没声明”），并触发 **`'卡牌-能否使用'`**（带上 `target` = 目标牌）。**目标牌由发起方给定**、不由内容侧枚举；`目标牌` 省略 = 只判“此刻能不能用” |
| `game:useCardToCard(使用者, 牌, 目标牌)` | **对一张牌使用**（`UseCardToCard`，`kind` = `useCardToCard`）：与 `useCard` 同一形状（**先 `canUseToCard` 校验**，不过就 `reject(原因)`；过了记一次使用 → 取牌 → `'卡牌-结算前'` + 牌自己的 `'结算前'` → **一次 `CardEffectToCard`**（跑 `'对卡牌生效'`）→ 牌自己的 `'结算后'` → `'卡牌-结算后'`）。目标固定是**一张牌**（不归一化成列表） |（阶段是内核概念，2026-09-22 落地；参数玩家在前）：建 `Phase` 实例 → 压栈 → `game.phase` 指向它 → 触发 **`'阶段-开始'`**（上下文 = 这个实例）→ 返回它。**返回的实例当 `<close>` 用**：作用域结束（`__close` → `Delete`）就触发 `'阶段-结束'`、出栈、`game.phase` 回到上一层；**阶段可嵌套**（栈，留给将来的「额外的一个出牌阶段」），离开必须按嵌套顺序（不是栈顶就报错）。内核只认「名字 + 属于谁」，不认识任何阶段名 |
...
| `on('对卡牌生效', handler)` | **固定钩子：“这张牌能被「对一张牌使用」”**（2026-09-24 落地）：**声明它就等于把这张牌放进「对牌使用」那一支** —— `canUseToCard` 的合法性就看这条（不像 `'获取目标'` 那样给目标表：这类目标**由发起方给定**，内容侧枚举不出来）；载荷 `(cardEffectToCard, useCard)`。与其它固定钩子同列，不是局上的时机 |
...
| `CardEffectToCard`（类，门面无工厂） | **对一张牌的一次生效**（`kind` = `cardEffectToCard`，与 `CardEffect` 对称、只是目标是牌）：字段 `def` / `useCard` / `user` / `card` / `target`（**目标牌**）；跑这张牌的 **`'对卡牌生效'`** 回调（载荷 `(cardEffectToCard, useCard)`）。【无懈可击】就是靠它被抵消的 —— 但它自己的钩子是个**空壳**（取消只能由被取消的效果自己的执行体发起，故抵消写在窗口里）；与别的效果一样有 `getTempZone()` 与 `'效果-收尾'`。与 `CardEffect` 一样逐目标？不 —— **一次「对牌使用」只产生一个 `CardEffectToCard`**（目标是单张牌）|
