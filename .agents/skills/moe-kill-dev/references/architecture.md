...
- 挂起模型：**一次结算 = 一个协程**，`Effect:apply()` 是这次结算**唯一的驱动点**（`coroutine.resume` 循环）；需要让出时用 `coroutine.yield(原因)`，由 `apply()` 收尾（目前只有「取消」一种让出，见 §12）。**不要**用 `ls.await.call` / `ls.await.yield` 驱动效果协程：前者把协程里的错误交给它的 `errorHandler`（吞掉）并且会丢掉我们的让出信号，后者把 resume 的所有权交给答复方、与「`apply()` 独占驱动」冲突。
...
- **命名用 `_` 前缀**：标明「内核内部状态，不是对外接口」。`moe` 是普通表，框架不会碰它，`_` 在这里没有额外含义。
