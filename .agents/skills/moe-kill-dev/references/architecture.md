...
| `game:canUseToCard(使用者, 牌, 目标牌?)` | **这张牌能不能「对一张牌使用」**（普通方法，与 `canUse` 平行、**同步且只读**）：牌本体那半与 `canUse` **共用同一段内核校验**，差别只在合法性依据 —— 要求**定义声明过 `'获取卡牌目标'`**（否则原因写成“没声明”）；**给了目标牌就拿它过一遍这条钩子**（返回空 ⇒ 不能对这张牌使用），再触发 **`'卡牌-能否使用'`**（带上 `target` = 目标牌）。**目标牌由发起方给定**、不由内容侧枚举；`目标牌` 省略 = 只判“这类牌能不能对牌使用” |
...
| `on('获取卡牌目标', handler)` | **固定钩子：“这张牌能对哪张牌使用”**（2026-09-24 从 `'对卡牌生效'` 改来）：载荷 `CardDef.Target`（比角色支多一条 `target` = 发起方**要对的那张牌**），**返回那张牌 = 认，返回空 = 不能对这张牌使用**（多个声明都认才通过）；**声明它 = 这张牌进「对牌使用」那一支** —— `canUseToCard` 的合法性就看它（不像 `'获取目标'` 那样枚举目标：这类目标**由发起方给定**，内容侧枚举不出来）。目标牌省略（只判「这类牌能不能对牌使用」）时它不跑。与其它固定钩子同列，不是局上的时机 |
