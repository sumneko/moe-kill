...
- **攻击范围与距离**（2026-09-24 接上修正）：属性 `攻击范围`（默认 1）与两条修正属性（`进攻修正` / `防御修正`，默认 0）都由**基础规则给每个玩家写入**；「够不够得着」由内容包拿**距离**与攻击范围组合得出 —— 内核不作任何具体判定（见 `package/标准/卡牌/杀.lua`：「除使用者以外、且在攻击范围内」，因此**打不到自己**）。
  ...
  - **装备牌的数据写在牌定义上**（`CardDef:value`，内核只存不解释）：武器写 `攻击范围` 的**增量**（麒麟弓 ⇒ `4`，因为默认 1）、坐骑写 `距离修正`（进攻马 `-1` / 防御马 `1`）；**槽位名不另写一份数据** —— 分类里带着它，**每个具体类别一个定义**（`武器牌` 带 `武器`；坐骑因为两种马「功能一模一样」而各给一个定义：`进攻马` / `防御马` 各自 `extends '坐骑牌'` 再补自己那条 kind），于是具体牌只写数据、一条 `: extends` 就行（`麒麟弓 : extends '武器牌'` / `赤兔 : extends '进攻马'`）；`装备牌.lua` 拿分类去 `装备区.slots` 里找自己那条。两个坐骑定义的**钩子是从 `坐骑牌` 抄来的**（`extends` 会带上基类的钩子），扫的是 `装备区.slots` 里的槽位名。
