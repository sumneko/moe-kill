...
| `game:askCard(被问者, 缘由, 条件?)` | **要一张牌**（`moe.askCard` 造 `AskCard : Effect`，`kind` = `askCard`）：返回**询问实例**（同样「驱动 + 等」），询问通过局上的 **`'卡牌-询问'`** 时机交给应答方，应答方调 `ask:answer{ card = 那张牌, targets = 目标 }`；**答复读 `.card` / `.targets`**（转存自 `.result`，后者是答复本体 `AskCard.Answer`），没人答上或被取消 ⇒ 「不存在」。**缘由**：`'使用'` 是内核**认识**的一个取值（按使用语义筛候选，见下），其余缘由仍不透明（只原样带给规则层）。**条件**是第三参数（`AskCard.Condition`）—— **每个字段都是一条筛选条件**：`name?` 牌名 / `zone?` 牌在哪个区里 / `card?` 牌必须在这批里 / `target?` 可用目标要与它至少有一个重合；**数组 = 满足其一，单值 = 当成只有一个的数组（归一化走 `moe.util.toList`），不填 = 无要求**。**候选从哪来**：给了 `zone` / `card` 就取那里（并集；`zone` 的**名字按「被问者 → 局上」解析**，别人的区要传区对象），都没给才遍历被问者名下的牌区；`name` / `target` 是纯筛选。**缘由是 `'使用'`（或给了 `target`）⇒ 候选逐张跑 `game:canUse`**：用不了的牌（没合法目标 / 次数用满 / 不在声明的牌区 / 内容侧 `'卡牌-能否使用'` 否决）不进选项，选项带 `targets` = 它的合法目标（给了 `target` 时取**交集**，于是应答方只能从名单里选）；**不给条件 = 不做限制**（不筛、不算目标）。**答复必须落在选项里**：牌不在选项里 / 选项带 `targets` 而答复没给目标、或给了名单外的目标 / 选项不带 `targets` 而答复给了目标 ⇒ **拒收**（`.err` = 原因、`.card` 为「不存在」，对调用方与"没答上"同一个分支）。**应答方给出了答复**时才触发 **`'卡牌-答复'`** 与紧接着的 **`'卡牌-答复后'`**（上下文 = 这次询问；没人应答不触发），规则层据此决定"这次答复意味着什么、那张牌怎么处置"（基础规则：缘由是「打出」⇒ 放进**发起这次打出的那次结算**的临时区，由它的收尾送进 `弃牌`） |
...
- **询问与应答方**：内容侧要一张牌就 `game:askCard(被问者, 缘由, 条件?)` —— 它是一次效果（有父效果、可被取消、`kind` = `askCard`），**答复就是这次询问的结果**（读 `.card` / `.targets`，**应答一给出就定下** —— 答复时机里 `askCard.card` 就读得到）。**缘由**（第二个参数）：`'使用'` 是内核**认识**的取值（按使用语义筛候选），其余缘由不解释、只原样带给规则层。**条件**（第三个）描述"要什么样的牌"：`name?` / `zone?` / `card?` / `target?` 四条筛选条件（数组 = 其一，不填 = 无要求），候选来源与使用语义见 `AskCard` 行；**业务层不自己拼候选**（"哪些牌可以给、答复在不在里面"得由内核统一做 —— 协议 / AI 给的答复是**外部输入**）。这里**没有"区域候选"这种东西**（2026-09-23 撤）：服务器侧看得见所有牌，暗区的遮蔽归协议层。内核**不写死「谁来答」**：询问通过局上的 **`'卡牌-询问'`** 时机送出去，应答方拿到询问实例后调 **`ask:answer{ card = …, targets = … }`**（**`targets` 给单个或一张列表都行** —— 入库前统一成列表，所以 `.targets` 恒为 `Player[]`；答不上就 `answer(nil)` 或干脆不调）；应答方可以**让出**（当场拿不到就等会儿再答，业务侧照旧像写同步代码一样写）；**没答上不是失败**（`ask.card` 为「不存在」），只有**重复应答**才记一条 `info`（先答的算数）。这一套只服务「要一张牌」的问法 —— 要技能 / 数字的问法将来另开一个类（`askSkill`…）。
