...
| `Card`（类，门面无工厂） | **一张牌**：字段 `id`（号由局发，私有 + `getId()`）/ **`name`（牌名，公开字段）** / **`suit`（花色）/ `point`（点数）** / 私有 `zone` / `game`（属于哪一局，建牌时给）；**牌名与牌面都是内容侧给的值，内核只存不解释**（不校验、不换算、不认识「红桃」也不认识 13 种点数）；**读牌名 / 花色 / 点数与读 `card.name` 一样都是直接读字段**（没有 getter，见 `code-style.md` 第 11 节），写牌名用 `setName(牌名)`；不在任何牌区时 `zone` 为空。**牌自己会读自己的内容定义**（2026-09-24 落地）：`getDef()` 给定义实例（不在局里 / 没定义 ⇒ 空）、`isKind(分类)` / `getValue(数据名)` 是它的转发、`fullName` 是只读派生属性（包名.名字）—— 于是**内容侧的钩子拿到的 `card` 就能直接问自己的分类与数据**（`card:isKind('武器')` / `card:getValue('攻击范围')`），不必再手工 `game:getCard(card.name)`；测试里自己造的牌没有局 ⇒ 这些都是空 |
