...
- 注入面只有 `game`（**这一局**，小写对象）+ `Card(name)` / `Depends(items)`（**大写环境函数**，只在本轮加载期可用；大写是为了和包里的局部变量区分开）+ 一份**标准库白名单**（`string` / `table` / `math` / `utf8` / `pcall` 等基函数）；`moe`（含 `moe.core` 这类写法）、`require`、`io`、`os` 一律不给 —— 规则集是内容，不该具备开文件 / 起进程的能力。
...
- **一局的资源直接用注入的 `game`**：牌与牌区是运行期的东西，按名字建在局上（`game:createZone(名字, 有序?)` / `game:createCard(名字)`），桌子与随机源是局上的公开字段（`game.desk` / `game.random`）。**环境对象就是 `game` 本身，不进时机上下文**（见第 10 节）。
...
| 接口 | 说明 |
| ---- | ---- |
| `moe.effect`（`Effect`） | **效果基类**：持局 + 种类标识 `kind` + 外层效果 `parent`；`apply()` = 记父（栈顶）→ 压入结算栈 → `settle()`（子类实现）→ 退栈 |
