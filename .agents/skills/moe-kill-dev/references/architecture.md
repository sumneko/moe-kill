...
| `game:askCard(被问者, 要什么牌)` | **要一张牌**（`moe.askCard` 造 `AskCard : Effect`，`kind` = `askCard`）：返回**询问实例**（同样「驱动 + 等」），询问通过局上的 **`'游戏-询问'`** 时机交给应答方，应答方调 `ask:answer(那张牌)`；给出的牌读 `.result`，没人答上或被取消 ⇒ 「不存在」 |
| `game:respond(角色, 牌)` | **打出一张牌**（与「使用」并列）：从该角色名下牌区取出 → 触发 `'卡牌-打出后'`（去向由内容侧定）；**不产生牌自身的效果** |
| `game:moveCard(牌, 区名)` | **把牌挪到某个牌区**（直接给单张牌与单个区名）：`card` 可以是单张或一批，`zone` 可以是单个区名或**一串区名**（一串 = 依次经过，停在最后一站）；按牌自己记的归属找源区（见下条）再移过去；区名不存在 / 某张牌不在任何牌区里 ⇒ 明确失败且不改状态 |
...
- **内核零区域名**：`UseCard` 不预设「手牌 / 弃牌」，它自己在使用者名下牌区里找这张牌；**牌的去向由内容侧决定** —— 结算完成后触发 `'卡牌-结算后'`，`@基础` 订阅它把牌放进 `弃牌`。
- **牌记自己的归属**（用户 2026-09-20 定）：`Card` 带 `zone` 字段（读 `card:getZone()`，不在任何牌区时为「不存在」），由牌区在 `put` / `take` / `move` / `clear` 时维护（写入走内核内部的 `card:bindZone`）。于是「**一张牌同时在两个区里**」不再成立：`put` 只接受不在任何牌区的牌（已经在某个区里 ⇒ 明确失败，换区用 `Zone:move`）。**只记「哪个牌区」、不记下标** —— 下标会被任何一次插入改掉（要下标自己 `zone:list()` 里找）。`game:moveCard` 就是靠它在全库范围内找牌。
...
- **询问与应答方**：内容侧要一张牌就 `game:askCard(被问者, 要什么牌)` —— 它是一次效果（有父效果、可被取消、`kind` = `askCard`），**给出的那张牌就是这次询问的结果**（`ask.result`），读法与 `game:damage` / `game:useCard` 一致。内核**不写死「谁来答」**：询问通过局上的 **`'游戏-询问'`** 时机（`game:on` / `game.events:on` 皆可）送出去，应答方拿到询问实例后调 **`ask:answer(那张牌)`**；应答方可以**让出**（当场拿不到就等会儿再答，业务侧照旧像写同步代码一样写）；**没答上不是失败**（`ask.result` 为「不存在」），只有**重复应答**才报错。「要什么牌」由内容侧定（内核不解释载荷）；这一套只服务「要一张牌」的问法 —— 要数字 / 选项之类的问法将来另开一个类。
- **效果即可等待的任务**：效果有完成信号 —— `settle()` 跑完 = 完成（**它的返回值就是这次结算的结果**，读 `.result`）、被取消 = 没有结果、出错 = 失败（读 `.err`）。`:apply()` **驱动**这次结算、`:await()` **等它结完**（都返回效果自己；已驱动 / 已结完就立刻返回，幂等）、`:remove()` 取消这一次生效。**入口（`game:useCard` / `game:damage` / `game:ask`）自动「驱动 + 等」并返回效果实例** —— 调用方必须等：效果是一条一条跑的链，同一时刻只有一条在跑。取消以「没有结果」完成、**不抛**；真故障记进 `.err`（`error` 只表示报错，不给「正常结束」当跳出用，见 `code-style.md` 第 10 节）。
- **打出**：`game:respond(角色, 牌)` 只做「取出 + 触发 `'卡牌-打出后'`」，**牌的去向仍由内容侧决定**（`@基础` 订阅它把牌放进 `弃牌`）。它与「使用」的差别是**不产生牌自身的效果**（不跑 `'生效'`、不触发 `'卡牌-结算后'`）。
...
Card '杀'
    ...
    : on('生效', function (ctx)
        ...
        local card   = game:askCard(target, { name = '闪' }).result   -- 要一张牌：给出的牌读 .result
        if card then                                                    -- 这里约定答复就是把那张闪本身
            game:moveCard(card, { '处理', '弃牌' })                            -- 去向由内容侧处置：一串区名 = 依次经过
            return                                                      -- 有闪就不受伤
        end
...
- **【2026-09-20 记录，待做】**：① `moveCard` 提成**效果**（`kind` = `moveCard`，一次结算挪**一批**牌，而不是现在逐张 `Zone:move`）—— 进结算栈、可挂时机、可被取消；② `game:askCard(被问者, reason, 要什么牌)` 加 `reason`（`'打出'` / `'使用'`，**内核不解释**取值），**基础规则据 `reason` 自动完成牌的移动**（打出 ⇒「处理 → 弃牌」）⇒ 业务层（内容包）**不再写 `moveCard`**；③ 顺带解决现在的重复：【闪】既被基础规则的 `'卡牌-打出后'` 送进 `弃牌`、又被内容包挪「处理 → 弃牌」；④ 处理区的牌**停留到什么时候**（该牌自己的响应结束 / 那一次生效结束）待定，与 ①② 一起做。
- 测试：`--test core.play`（校验 / 取牌 / 逐目标生效与行动顺序 / 单个生效可被取消 / 收尾时机 / 栈恢复 / 合法目标列表）、`--test core.effect`（效果链：父子 / 嵌套 / 结果与失败 / 取消 / 递归上限）、`--test core.ask-card`（询问往返 / 被问者与答复与父效果 / 多问互不串 / 没人应答 / 重复应答被拒 / **应答方让出后再答** / 被取消的询问；打出的取出与收尾时机）、`--test core.damage`、`--test core.move` / `--test core.game`（牌的归属：放进 / 取出 / 换区 / 清空，`put` 拒收已在区里的牌；`game:moveCard` 的单张 / 一批 × 单个 / 一串区名与几种明确失败）、`--test rule.slash`（「杀」端到端：攻击范围 / 造成伤害 / 进弃牌 / 不能对自己用 / 打出闪就不受伤 / 多目标依次）。
