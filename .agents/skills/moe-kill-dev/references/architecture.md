...
| `game:play(使用者, 牌, 目标们)` | **使用一张牌**（`UseCard`，`kind` = `useCard`）：校验（牌在手 / 目标是合法目标的非空子集）→ 取出牌 → **按行动顺序对每个目标各结算一次生效** → 触发收尾时机；等价于「造 `UseCard` + `:apply()`」 |
