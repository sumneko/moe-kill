...
据此，`Zone` 的通用 `params` 袋（2026-09-23 删）方向是错的：它把「功能性信息」降级成不透明数据。**实际照这个口径落地的是可见性**（2026-09-23）：`Zone.visible`（一等字段，默认 `true`）+ `Zone.owner`（玩家建区时自动记）+ `zone:setVisible(值)` / `zone:isVisibleTo(视角)` —— 内核给字段、内容侧给取值（哪个区是暗的、谁看得见）。
...
| `game:askCard(被问者, 缘由, 条件?)` | **要一张牌**（`moe.askCard` 造 `AskCard : Effect`，`kind` = `askCard`）：返回**询问实例**（同样「驱动 + 等」），询问通过局上的 **`'卡牌-询问'`** 时机交给应答方，应答方调 `ask:answer{ card = 那张牌, targets = 目标 }`（**候选是区域时**改成 `ask:answer{ zone = 那个区 }`）；**答复读 `.card` / `.targets` / `.zone`**（转存自 `.result`，后者是答复本体 `AskCard.Answer`），没人答上或被取消 ⇒ 「不存在」。**缘由**是不透明字符串（内核不解释、只原样带给规则层）；**条件**是第三参数（`AskCard.Condition` = `name?` 牌名 + `targets?` 目标窗口 + `cards?` 候选就是这批牌 + **`zones?` 候选还可以是这几个区整体**）—— **内核在询问之前算出 `ask.options`**（`AskCard.Option[]` = 牌 + 它的可用目标，或一个区；业务层不自己拼候选）：默认**遍历被问者名下每个牌区**按条件筛，**给了 `cards`（或 `zones`）就只看那些**（选“不属于任何人的牌”与选“别人区域里的牌”都靠它）。**`cards` 与 `zones` 可以同时给** ⇒ **一次询问**就说完了「选一张你看得见的牌，或者盲选某个看不见的区（“里面有什么”不暴露）」（【过河拆桥】/【顺手牵羊】的写法：选项顺序是**牌在前、区在后**，暗区只能给区、**不能逐张给牌** —— 给了牌就等于把暗牌给应答方看了）。条件里**给了 `targets` 就跑 `game:canUse`**（于是内容侧的 `'卡牌-能否使用'` 条目一并生效：用不了的牌根本不会进选项），**`targets = {}`（空表）= 只要求「至少有一个合法目标」**（出牌阶段就这么调：选项带各自的完整可用目标）；`targets` 非空 = 窗口（只收能用在这组人身上的牌，选项的目标就是这组人）；**省略 `targets` = 不要求目标**（不跑 `canUse`、不算目标 —— 如「打出」）；**不给条件 = 不做限制**。**答复必须落在选项里**：牌不在选项里 / 区不在选项里 / 选项带 `targets` 而答复没给目标、或给了窗口外的目标 / 选项不带 `targets` 而答复给了目标 ⇒ **拒收**（`.err` = 原因、`.card` 为「不存在」，对调用方与"没答上"同一个分支）。**应答方给出了答复**时才触发 **`'卡牌-答复'`** 与紧接着的 **`'卡牌-答复后'`**（上下文 = 这次询问；没人应答不触发），规则层据此决定"这次答复意味着什么、那张牌怎么处置"（基础规则：缘由是「打出」⇒ 放进**发起这次打出的那次结算**的临时区，由它的收尾送进 `弃牌`） |
...
- **牌区知道「谁能看见」**（2026-09-23 落地）：`Zone` 带 `visible`（默认 `true` = 对所有人可见）与 `owner`（**玩家建区时由 `player:addZone` 自动记**，公共区没有）⇒ `zone:setVisible(false)` 就是「只有持有者看得见」（**没有归属的暗区谁都不看不见** —— 抽牌堆将来的形状），`zone:isVisibleTo(视角)` 给出结论。读派只需这个结论，**不必自己写“谁的区默认暗”这类规则**。两个使用先例：【过河拆桥】/【顺手牵羊】把目标身上 **看得见的牌逐张**当候选项、**看不见的区整个**当一个候选项（见 `AskCard` 行）；可见性将来还给明牌技能、延时锦囊与判定区使用。
...
- **询问与应答方**：内容侧要一张牌就 `game:askCard(被问者, 缘由, 合法选项)` —— 它是一次效果（有父效果、可被取消、`kind` = `askCard`），**答复就是这次询问的结果**（读 `.card` / `.targets` / `.zone`，**应答一给出就定下** —— 答复时机里 `askCard.card` 就读得到）。**缘由**（第二个参数）是不透明字符串（如 `'打出'`）：内核**不解释**，只原样带给规则层。**合法选项**（第三个）是"这次可以给哪些牌"，写成一份**条件**（`AskCard.Condition`：`name?` 牌名 + `targets?` 目标窗口 + `cards?` 一批牌 + `zones?` 几个区）：**内核在询问前遍历被问者的牌区、按条件把选项算好挂在询问上**（业务层不自己拼）—— 内核仍然零语义、零区域名（它不认识牌名，只是拿 `card:getLabel()` 对一下；也不知道哪个区是手牌，只知道"被问者名下的牌"），但"哪些牌可以给、答复在不在里面"这两件事得由内核统一做（协议 / AI 给的答复是**外部输入**）。条件是**描述**、不是代码：只服务「要一张牌 / 一个区」的问法。内核**不写死「谁来答」**：询问通过局上的 **`'卡牌-询问'`** 时机送出去，应答方拿到询问实例后调 **`ask:answer{ card = …, targets = … }`** 或 **`ask:answer{ zone = … }`**（**`targets` 给单个或一张列表都行** —— 入库前统一成列表，所以 `.targets` 恒为 `Player[]`；答不上就 `answer(nil)` 或干脆不调）；应答方可以**让出**（当场拿不到就等会儿再答，业务侧照旧像写同步代码一样写）；**没答上不是失败**（`ask.card` 为「不存在」），只有**重复应答**才记一条 `info`（先答的算数）。这一套只服务「要一张牌 / 一个区」的问法 —— 要技能 / 数字的问法将来另开一个类（`askSkill`…），届时 `AskCard` 名字与"要区域"的关系一并谈。
