...
| `game:moveCard(牌们, 牌区名)` | **把牌挪进某个牌区**：按牌自己记的归属找源区（见下条）再移过去；目标区不存在 / 某张牌不在任何牌区里 ⇒ 明确失败且不改状态 |
...
Card '杀'
    ...
    : on('生效', function (ctx)
        ...
        if card then                                                    -- 这里约定答复就是把那张闪本身
            game:moveCard({ card }, '弃牌堆')                            -- 去向由内容侧处置（按牌的归属找源区）
            return                                                      -- 有闪就不受伤
        end
