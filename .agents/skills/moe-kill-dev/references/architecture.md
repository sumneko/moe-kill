...
规则侧第一个功能点：让一张牌能「用出去并产生后果」。机制在内核、规则全在内容包 —— 契约见 `server/test/core/effect.lua`、`server/test/core/play.lua` 与 `server/test/core/damage.lua`。
