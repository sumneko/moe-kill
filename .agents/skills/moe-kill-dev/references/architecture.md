...
| `Card:on(事件, 回调)` | 链式登记回调，返回同一条定义；**跨文件追加回调用 `game:getCard` + `Depends` 定顺序**。事件名是**固定集合**（`'目标合法'` / `'使用'`），清单在 `env-meta.lua` |
...
- **两类事件名，兜底策略不同**（用户 2026-09-19 定）：
  ...
  - **牌的钩子**（`CardDef:on`）是**固定集合**：名字由内核约定、与启用的包无关（也没有扩展需求），清单以 `server/core/loader/env-meta.lua` 为准 ⇒ **不留兜底**，拼错在编辑期直接报（实测：写 `: on('目标合法吗', …)` 会报「不能将 `string` 赋给参数 `'使用'|'目标合法'`」）。新加钩子**必须同步补进 `env-meta`**，钩子清单只有它一个事实来源。
...
| 接口 | 说明 |
| ---- | ---- |
| `moe.effect`（`Effect`） | **效果基类**：持局 + 种类标识 `kind`；`apply()` = 压入结算栈 → `settle()`（子类实现）→ 退栈 |
| `game:play(使用者, 牌, 目标们)` | **使用一张牌**（`UseCard`，`kind` = `useCard`）：校验（牌在手 / 目标合法）→ 取出牌 → 按声明顺序跑「使用」回调 → 触发收尾时机；等价于「造 `UseCard` + `:apply()`」 |
...
- **内容侧的挂法**：牌的定义上按内容侧事件名登记 —— `Card '杀' :on('目标合法', fn) :on('使用', fn)`；`'目标合法'` 返回 `false` 表示不合法（其它返回值视为通过），不合法时使用失败且牌留在原处；回调的上下文就是**这次使用的效果实例**（`UseCard`，字段 `user` / `card` / `targets`）。钩子名固定（清单以 `env-meta.lua` 为准，拼错编辑期报错），**内核不为钩子名加专门方法**。
...
- 测试：`--test core.play`（校验 / 取牌 / 结算回调 / 收尾时机 / 栈恢复）、`--test core.effect`（效果与结算栈：压退 / 嵌套 / 抛错也退 / 快照 / 撤销函数 / 深度上限）、`--test core.damage`（伤害实例 / 改体力 / 可为负 / 时机先后与同一个实例 / 结算期间在栈上）、`--test rule.slash`（「杀」端到端：攻击范围 / 造成伤害 / 进弃牌堆）。
