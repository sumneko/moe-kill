...
| `game:moveCard(牌, 区名)` | **把牌挪到某个牌区**（`moe.moveCard` 造 `MoveCard : Effect`，`kind` = `moveCard`）：`card` 可以是单张或一批，`zone` 可以是单个区名或**一串区名**（一串 = 依次经过，停在最后一站）；**一次结算把整批牌挪完**、返回效果实例（失败读 `.err`）；区名不存在 ⇒ 失败且不改状态；**没有归属的牌也能挪**（路径的第一站当作把它放进去） |
