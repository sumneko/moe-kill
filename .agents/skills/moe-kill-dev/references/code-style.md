...
- **例外一 = 接口的一行说明**：**入口**（公开/内核的函数、方法、字段、模块门面）保留**一行极短的中文说明**，几个字即可 —— 如 `---@field title string # 获取标题`、`---@return Card # 新牌`；贴在类型注解的 `#` 后面，**不另起 `--` 注释行**。
  - **接口说明写给「用这个接口的人」，不写内部实现原理**（用户 2026-09-24 定）：`---` 上面的那行、以及 `#` 后面的说明，说的是**调用方能拿到什么 / 要守什么约定**（`放进去就读得到`、`没声明过就是「不存在」`、`取值由你定`）。**解释内部怎么实现、为什么这么实现的话一律不写在函数上** —— 真要写就写进**函数体内**（紧贴那段代码的 `--`）；**与调用方无关的一律不写**（用户 2026-09-24：`canUse` 上那句「① 牌本身能不能用、② 给的目标配不配（两件事分开判）」就该直接删掉）。判断标准：这句话对**写内容包的人**有没有用？没有就删或挪进函数体。
  - **设计取舍与实现原理的完整解释归 `openspec`**（用户 2026-09-24）：变更的 `design.md` 才是记「为什么这么设计、怎么实现」的地方 —— **不要往代码注释（函数上或函数体）里再抄一遍**；代码里只留「看代码看不出来的意图」，一句话即可。
  - **`---@class` 是例外：它不吃行尾 `#`**（用户 2026-09-23 实测）—— 类的说明写在**上一行**，写成 `--- 说明`，例如：
...
| **不检查**：内核自己保证存在的 | 内核建好的**五个基础牌区**（局上 `抽牌` / `弃牌`，玩家身上 `手牌` / `装备` / `判定`，见 `architecture.md` 第 12 节）—— 区名也是对包作者的契约、`getZone` 的重载把它们收窄成非空 ⇒ 取出来直接用，**不要再 `assert`**（要新区用 `player:addZone`） |
...
- **存量不动**：测试里那批 `assert(x:getZone('手牌'))`（2026-09-24 清 `package/` 时扫出 **143 处、18 个测试文件**）不主动清 —— 纯噪音改动，顺手遇到相关代码时再单独提。
