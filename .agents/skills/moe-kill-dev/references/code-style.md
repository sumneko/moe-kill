...
| 要不要检查 | 例子 |
| ---- | ---- |
| **不检查**：调用约定 / 类型契约已经保证的 | 规则实例给的场地（`rule.room`：实例由场地建出来，一定带着场地）、自己模块内互相调用的参数、自己创建的对象 |
| **不检查**：防自己人篡改 | 不校验「我们自己定义的函数 / 对象会不会被使用者替换」—— 那不是本工程要支持的用法；我们相信 `rule.room` 拿到的是我们自己定义的场地 |
