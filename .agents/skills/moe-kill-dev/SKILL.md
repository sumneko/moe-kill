...
| `server/core/effect/` | **效果族**：`Effect` 与它的**全部子类**都住这里（判断标准就一条 —— *是不是效果*；新加效果也放这儿）。`init.lua` = 基类 `Effect`（由任务驱动 —— `apply()` 驱动 → `settle()` 结算，结果 / 失败读 `.result` / `.err`，父效果读 `.parent`）；子类：`use-card.lua`（`UseCard` + 逐目标 `CardEffect`）、`ask.lua` / `ask-card.lua`（询问）、`move-card.lua`（挪牌）、`damage.lua` / `heal.lua`（伤害 / 回复：只触发前·生效·后三个时机，**改体力属规则侧**）、`draw.lua`（摸牌：只触发 `'摸牌'`）、`dying.lua`（濒死：只触发 `'濒死'`，判死归规则；`game:enterDying` 记账 + 结算收尾才起）；**模块名 = 路径**（基类仍叫 `core.effect`，子类是 `core.effect.damage` …） |
