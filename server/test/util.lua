...
lt.test('工具：toList 把单个值与列表统一成列表', function ()
    ...
    local single = assert(moe.util.toList(card))
    lt.assertEquals('单个值包成一张表', 1, #single)
    lt.assertEquals('包起来的就是那个值', card, single[1])

    lt.assertEquals('已经是列表就原样返回（同一个表）', list, assert(moe.util.toList(list)))
    lt.assertEquals('列表原样返回后长度不变', 2, #list)
    lt.assertEquals('空表也是空列表（原样返回）', empty, assert(moe.util.toList(empty)))
end)
