...
lt.test('包：跨包同名并存', function ()
    local guard <close> = prepare()
    write('标准/卡牌/杀.lua', 'Card("杀")')
    write('军争/卡牌/杀.lua', 'Card("杀")')

    load(list('标准', '军争'))

    lt.assertEquals('两个包各有一条', true, game:getCard('标准.杀') ~= nil and game:getCard('军争.杀') ~= nil)
    lt.assertEquals('两条不是同一个对象', true, game:getCard('标准.杀') ~= game:getCard('军争.杀'))
end)
...
lt.test('路由：包内裸名优先本包', function ()
    ...
    write('标准/卡牌/杀.lua', 'Card("杀")')
    write('军争/卡牌/杀.lua', 'Card("杀")\n'
        .. 'local own = game:getCard("杀")\n'
        .. 'Card("检查"):on("跑", function () return own.fullName end)')
...
lt.test('路由：本包没有时 fallback', function ()
    local guard <close> = prepare()
    write('标准/卡牌/闪.lua', 'Card("闪")')
    write('军争/卡牌/火杀.lua', 'local dodge = game:getCard("闪")\n'
        .. 'Card("检查"):on("跑", function () return dodge and dodge.fullName or "没有取到" end)')
...
lt.test('路由：限定名不受包内优先影响', function ()
    ...
    write('标准/卡牌/杀.lua', 'Card("杀")')
    write('军争/卡牌/杀.lua', 'Card("杀")\n'
        .. 'local fromStandard = game:getCard("标准.杀")\n'
        .. 'Card("检查"):on("跑", function () return fromStandard.fullName end)')
...
lt.test('路由：两种写法是同一个对象', function ()
    ...
    lt.assertEquals('裸名与限定名是同一个对象', true, card('军争.杀') == game:getCard('军争.杀'))
    lt.assertEquals('包外裸名取的是先加载的包', true, card('杀') == game:getCard('标准.杀'))
end)
...
lt.test('路由：未登记的名字查不到', function ()
    ...
    lt.assertEquals('裸名查不到', nil, game:getCard('闪'))
    lt.assertEquals('限定名查不到', nil, game:getCard('军争.杀'))
    lt.assertEquals('不存在的包查不到', nil, game:getCard('没有这个包.杀'))

    ---@type any
    local notString = nil
    lt.assertError('名字必须是字符串', function ()
        game:getCard(notString)
    end)
end)
