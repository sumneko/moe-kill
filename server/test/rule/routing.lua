...
lt.test('包：跨包同名并存', function ()
    ...
    rule:load(list('标准', '军争'))

    lt.assertEquals('两个包各有一条', true, rule:getCard('标准.杀') ~= nil and rule:getCard('军争.杀') ~= nil)
    lt.assertEquals('两条不是同一个对象', true, rule:getCard('标准.杀') ~= rule:getCard('军争.杀'))
end)
...
lt.test('路由：两种写法是同一个对象', function ()
    ...
    lt.assertEquals('裸名与限定名是同一个对象', true, card('军争.杀') == rule:getCard('军争.杀'))
    lt.assertEquals('包外裸名取的是先加载的包', true, card('杀') == rule:getCard('标准.杀'))
end)
...
lt.test('路由：未登记的名字查不到', function ()
    ...
    lt.assertEquals('裸名查不到', nil, rule:getCard('闪'))
    lt.assertEquals('限定名查不到', nil, rule:getCard('军争.杀'))
    lt.assertEquals('不存在的包查不到', nil, rule:getCard('没有这个包.杀'))

    ---@type any
    local notString = nil
    lt.assertError('名字必须是字符串', function ()
        rule:getCard(notString)
    end)
end)
