...
lt.test('时机：加载期注册的回调能被触发', function ()
    ...
    write('甲/开始.lua', 'rule:on("游戏-开始", function (ctx) ctx.记录 = "甲" end)')

    moe.rule.load(list('甲'))

    ---@type table<string, any>
    local ctx = {}
    moe.rule:fire('游戏-开始', ctx)
...
lt.test('时机：后注册的后执行，于是覆盖先前的', function ()
    local guard <close> = prepare()
    write('甲/开始.lua', 'rule:on("游戏-开始", function (ctx) ctx.身份 = "甲写的" end)')
    write('乙/开始.lua', 'rule:on("游戏-开始", function (ctx) ctx.身份 = "乙写的" end)')
...
lt.test('时机：注册返回的 disposer 能撤销', function ()
    local guard <close> = prepare()
    write('甲/开始.lua', 'local undo = rule:on("游戏-开始", function (ctx) ctx.记录 = "甲" end)\n'
...
lt.test('时机：加载之外不能注册', function ()
    lt.assertError('加载之外注册报错', function ()
        moe.rule:on('游戏-开始', function () end)
    end)
end)

lt.test('时机：清空重载后旧注册不再触发', function ()
    local guard <close> = prepare()
    write('甲/开始.lua', 'rule:on("游戏-开始", function (ctx) ctx.记录 = "第一轮" end)')

    moe.rule.load(list('甲'))
    ---@type table<string, any>
    local first = {}
    moe.rule:fire('游戏-开始', first)
...
lt.test('时机：同一个时机在多个文件里注册也按加载顺序执行', function ()
    local guard <close> = prepare()
    write('甲/一.lua', 'rule:on("游戏-开始", function (ctx) ctx.顺序 = (ctx.顺序 or "") .. "一" end)')
    write('甲/二.lua', 'rule:on("游戏-开始", function (ctx) ctx.顺序 = (ctx.顺序 or "") .. "二" end)')
