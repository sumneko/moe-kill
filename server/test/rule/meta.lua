...
lt.test('互斥：没被加载时不报错', function ()
    ...
    local loaded = moe.rule.load(list('甲'))

    lt.assertEquals('加载正常完成', 1, #loaded)
    lt.assertEquals('条目照常登记', true, moe.rule.getCard('甲牌') ~= nil)
end)
...
lt.test('互斥：前缀相同但不同包不误判', function ()
    ...
    moe.rule.load(list('甲', '乙外传'))

    lt.assertEquals('两个包都登记成功', true, moe.rule.getCard('甲牌') ~= nil and moe.rule.getCard('外传牌') ~= nil)
end)
...
lt.test('互斥：冲突时本轮没有任何文件被执行', function ()
    ...
    lt.assertEquals('先有一轮成功的加载', true, moe.rule.getCard('旧牌') ~= nil)
    ...
    lt.assertEquals('本轮声明方没执行', nil, moe.rule.getCard('新牌'))
    lt.assertEquals('本轮被排斥方也没执行', nil, moe.rule.getCard('被排斥的牌'))
    lt.assertEquals('上一轮的内容没被破坏（校验期失败不执行任何文件）', true, moe.rule.getCard('旧牌') ~= nil)
end)
...
lt.test('同包重复条目：执行前就报错并指出两处来源', function ()
    ...
    lt.assertEquals('执行前就报错，本轮没有文件被执行', nil, moe.rule.getCard('重复的牌'))
end)
...
lt.test('元信息：依赖、条目、文件与来源都可查', function ()
    ...
    local meta = assert(moe.rule:getPackageMeta('甲'), '没有拿到包元信息')
    ...
    lt.assertEquals('查不到的包返回不存在', nil, moe.rule:getPackageMeta('没有这个包'))
    lt.assertEquals('可以一次取全部元信息', true, moe.rule:getMetas()['甲'] ~= nil)
end)
...
lt.test('元信息：返回的是只读快照', function ()
    ...
    local again = assert(moe.rule:getPackageMeta('甲'), '没有拿到包元信息')
...
lt.test('互斥：指向不存在的包时也不报错', function ()
    ...
    lt.assertEquals('加载正常完成', true, moe.rule.getCard('甲牌') ~= nil)
end)
