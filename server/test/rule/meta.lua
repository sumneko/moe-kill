...
lt.test('互斥：前缀相同但不同包不误判', function ()
    local guard <close> = prepare()
    write('甲/开始.lua', 'Depends { "!乙" }\nCard("甲牌")')
    write('乙外传/牌.lua', 'Card("外传牌")')

    load(list('甲', '乙外传'))

    lt.assertEquals('两个包都登记成功', true, game:getCard('甲牌') ~= nil and game:getCard('外传牌') ~= nil)
end)

lt.test('互斥：冲突时本轮没有任何文件被执行', function ()
    local guard <close> = prepare()
    write('甲/牌.lua', 'Card("旧牌")')
    load(list('甲'))
    lt.assertEquals('先有一轮成功的加载', true, game:getCard('旧牌') ~= nil)

    write('甲/牌.lua', 'Depends { "!乙" }\nCard("新牌")')
    write('乙/牌.lua', 'Card("被排斥的牌")')

    lt.assertError('互斥冲突报错', function ()
        load(list('甲', '乙'))
    end)

    lt.assertEquals('本轮声明方没执行', nil, game:getCard('新牌'))
    lt.assertEquals('本轮被排斥方也没执行', nil, game:getCard('被排斥的牌'))
    lt.assertEquals('上一轮的内容没被破坏（校验期失败不执行任何文件）', true, game:getCard('旧牌') ~= nil)
end)

lt.test('同包重复条目：执行前就报错并指出两处来源', function ()
    local guard <close> = prepare()
    write('甲/一.lua', 'Card("重复的牌")')
    write('甲/二.lua', 'Card("重复的牌")')
    ...
    lt.assertEquals('执行前就报错，本轮没有文件被执行', nil, game:getCard('重复的牌'))
end)

lt.test('元信息：依赖、条目、文件与来源都可查', function ()
    local guard <close> = prepare()
    write('甲/开始.lua', 'Depends { "./牌" }\nCard("甲牌")')
    write('甲/牌.lua', 'Card("乙牌")')

    load(list('甲'))

    local meta = assert(game:getPackageMeta('甲'), '没有拿到包元信息')
    ...
    lt.assertEquals('查不到的包返回不存在', nil, game:getPackageMeta('没有这个包'))
    lt.assertEquals('可以一次取全部元信息', true, game:getMetas()['甲'] ~= nil)
end)

lt.test('元信息：返回的是只读快照', function ()
    local guard <close> = prepare()
    write('甲/牌.lua', 'Card("甲牌")')
    ...
    local again = assert(game:getPackageMeta('甲'), '没有拿到包元信息')
...
lt.test('预解析：试跑失败只记警告，加载照常完成', function ()
    local guard <close> = prepare()
    write('甲/一.lua', 'Card("甲牌")')
    write('甲/二.lua', 'assert(game:getCard("甲牌") ~= nil, "还没到时候")')
...
lt.test('预解析：跨文件依赖在元信息里完整', function ()
    local guard <close> = prepare()
    write('甲/主.lua', 'Depends { "./深层/牌" }')
    write('甲/深层/牌.lua', 'Card("深牌")')
...
lt.test('互斥：指向不存在的包时也不报错', function ()
    local guard <close> = prepare()
    write('甲/牌.lua', 'Depends { "!没有这个包" }\nCard("甲牌")')

    load(list('甲'))

    lt.assertEquals('加载正常完成', true, game:getCard('甲牌') ~= nil)
end)
