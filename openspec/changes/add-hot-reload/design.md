...
### D10 撤销与自动注销的分工（用户 2026-09-19 补充）

上游靠「模块被重载时它的回调自动注销」避免回调堆积，这个默认路径我们保留：**可重载模块里、跟着该模块走的注册不写 disposer**。

D1 里说要给回调加 disposer（项目约定：订阅类接口返回撤销函数）—— 接口形状保留，但**用途收窄**为两类：

1. 注册发生在**不可重载**的模块里（回调的 `name` 为 `nil`，永远不会被自动注销）；
2. 自动注销**盖不到、必须重建或显式释放**的资源（外部系统注册、timer、socket、缓存对象…）—— 这类用 `onBeforeReload` 捕获引用清理、`onAfterReload` 重建，或直接用上游的 `recycle(cb)`。

所以规格里「注册回调 MUST 返回撤销函数」是**接口契约**，并不意味着每次注册都要调用它；文档（`architecture.md` 第 8.5 节）按这个口径写。
