...
- **去掉忙等**：事件循环空闲时改为**阻塞等待到「下一个定时器到期时间」**（无定时器时按上限分片等待），并删除 `busyTime` / `markBusy` / `getIdleTime` 这套忙等启发式。等待由注入的 waiter 承担，`tools/` 不反向依赖上层模块。
...
非目标：不引入子线程 / 工作线程；不做网络传输层与 socket 业务（只做唤醒接线）；不改协议与游戏逻辑。
