...
- [ ] 9.1 `server/core/zone.lua`：`zone:move(card, to, position?)` —— 源区持有该牌 / 两端启用 / 位置合法三项先校验完再改状态，失败时两边计数与顺序都不变；返回被移动的牌
- [ ] 9.2 目标位置：整数，`1` = 顶、`-1` = 底、省略视同底部，`-k` 为倒数第 k 位；越界或非整数明确失败；无序牌区同样接受
- [ ] 9.3 同区移动 = 重排（不产生重复的牌）
- [ ] 9.4 薄模型：内核不记录牌的归属，`put` / `take` 语义不变，移动不产生事件与记录
- [ ] 9.5 用例 `server/test/core/move.lua`：跨区移动、位置语义（顶/底/中间/省略/负数）、同区重排、牌不在源区失败且状态不变、任一端禁用失败、位置越界与类型错误、同区与跨有序区的组合
