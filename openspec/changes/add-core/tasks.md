...
**交付方式（用户 2026-09-19 定）**：一个功能点做完就与用户对齐一次。实际轮次：① 改名 core + 照搬 attribute → ② `core-random` → ③ `core-zones` → ④ `core-attributes` → ⑤ `core-move`（牌的移动，临时插入）→ 之后 `core-objects` 按需补 → 收尾文档与验收。
...
- [ ]（**已移出本变更**，随对象模型另开提案）4.4b 玩家侧牌区的动态添加 / 删除与“默认列举不含被禁用牌区”
...
- [ ]（**已移出本变更**，待有消费者）5.5 公式 / 复杂属性（`setFormula` / `setBaseSymbol` / 字符串型 `min`-`max` 引用）与“查询自上次检查以来的变化”（库的 `getTouched`，返回的是变化**前**的取值）

## 6. 对象模型（`core-objects`）—— **已移出本变更**（届时另开提案，见 design.md 附录）

- [ ] 6.1 `server/core/player.lua`：持有属性实例、参与行动标记、不透明标签、一组可增删禁用的牌区
- [ ] 6.2 `server/core/desk.lua`：入座、座位序号、按行动顺序遍历（跳过不参与行动的玩家）、相邻座位
...
- [x] 7.1 新增 `server/test/core/init.lua` 并挂到测试入口的套件列表
- [x] 7.2 覆盖随机源：同种子同序列、不同种子不同序列、实例间互不干扰
- [x] 7.3 覆盖牌区：放入 / 取出 / 计数 / 列举 / 清空、空区取牌明确失败、参数设置与修改、禁用启用、相同随机源洗出相同顺序、按顺序取牌、无序牌区取顶失败、牌实例标识可区分（“玩家侧动态增删”那半随 4.4b）
- [x] 7.4 覆盖属性：自定义属性名、上下限约束（含增减越界）、实例独立、变更可观察、实例创建后新增定义失败
- [ ]（**已移出本变更**，随对象模型）7.5 覆盖对象：属性承载数值、参与标记与标签、牌区增删禁用、行动顺序跳过不参与者、座位距离与修正下限、两个房间互不影响
- [x] 7.6 覆盖“调用方组合接口搭场景”：用手动取牌 + 放入牌区的方式发出一手牌并断言计数 —— 证明内核不提供“发牌”也能搭出场景，且内核中不存在预设区域名/属性名（`server/test/core/scenario.lua`）

## 8. 验收与文档

- [x] 8.1 `luamake -notest` 编译通过；`server/bin/moe-kill.exe --test` 全绿、退出码 0；问题面板 information 及以上归零
- [x] 8.2 `architecture.md`：分层图补 `core`（内核，与规则无关）与 `game`（规则集，调用内核）；「无头可测」补"内核接口可直接调用、调用方自行组合场景"；数据流图里的决策挂起改到 Room
- [x] 8.3 `sanguosha-rules` 技能：把「决策点统一走请求输入 → 挂起 → 恢复」的主语改为 **Room 内部**（后续批次实现），并注明**内核不含任何流程且不预设区域名/属性名**
- [x] 8.4 `setup-backend-infra/design.md` 里过期的"`script/engine/` 未来放纯规则引擎"一句修正为 `server/core` + `game/` 的新划分（含 `script/` → `server/` 改名）
- [x] 8.5 清理 `tmp/`、`log/` 中的临时产物

## 9. 牌的移动（`core-move`，本轮临时插入）

- [x] 9.1 `server/core/zone.lua`：`zone:move(card, to, position?)` —— 源区持有该牌 / 两端启用 / 位置合法三项先校验完再改状态，失败时两边计数与顺序都不变；返回被移动的牌
- [x] 9.2 目标位置：整数，`1` = 顶、`-1` = 底、省略视同底部，`-k` 为倒数第 k 位；越界或非整数明确失败；无序牌区同样接受
- [x] 9.3 同区移动 = 重排（不产生重复的牌）
- [x] 9.4 薄模型：内核不记录牌的归属，`put` / `take` 语义不变，移动不产生事件与记录
- [x] 9.5 用例 `server/test/core/move.lua`：跨区移动、位置语义（顶/底/中间/省略/负数）、同区重排、牌不在源区失败且状态不变、任一端禁用失败、位置越界与类型错误、同区与跨有序区的组合
