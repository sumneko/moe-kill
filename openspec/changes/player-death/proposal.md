...
- **「参与行动」改成派生属性**：`player.acting` 由 `__getter` 现算，当前**只检查存活**（活着就参与行动，阵亡即退出行动顺序）；原 `setActing` / `isActing` 删除（一个概念不要两套 API，也不再让调用方直接置位）。将来出现「退出 / 旁观 / 未参与本局」等其它情形时再扩展派生条件。
- **本批不做**：濒死流程（体力归零 → 求桃 → 才判定死亡）、死亡结算 / 奖惩 / 胜负判定。
...
- `core-player`: 新增**存活状态**（`isAlive` / `setAlive`）与**死亡时机**的触发口径、「玩家在入座或局建立时获得所属局」；并把「参与行动」改成**由存活派生的只读属性**
