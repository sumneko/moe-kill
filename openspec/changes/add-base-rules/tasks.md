...
- [ ] 3.2 注入面只保留 `rule`（`core` / `require` / `io` / `os` 都拿不到），并在 `rule` 上补四个最小工厂：`createAttributeSystem()` / `createCard(name)` / `createZone()` / `createOrderedZone()`；验证：规则集里能调这四个工厂的用例通过、写 `core.` 会报错的用例通过、既有「拿不到 require」用例继续通过
...
- [ ] 3.5 `server/rule/env-meta.lua` 去掉 `core` 的全局声明；验证：`package/` 下写 `core.` 报未定义全局、问题面板不残留

## 4. 规则包

- [x] 4.1 `package/基础/配置.lua`：用 `rule:setValues` 落默认值（体力上限 4、主公体力上限加成 1 等）；验证：读取到默认值的用例通过
- [ ] 4.2 `package/基础/体力.lua`：用 `rule:createAttributeSystem()` 建属性系统，定义 `体力` / `体力上限` 两个属性；验证：属性可用的用例通过，且文件里不出现 `core.`
- [ ] 4.3 `package/基础/牌堆.lua`：注册「游戏-开始」，用 `rule:createCard(名字)` / `rule:createOrderedZone()` 按牌表建牌实例 → 放入有序牌区 → 用注入的随机源洗牌；牌表缺失时报错（不写牌堆）；验证：三个用例通过，且文件里不出现 `core.`
...
- [ ] 6.1 `architecture.md`：第 9 节补「执行环境注入面（只有 `rule`，MUST NOT 注入 `core`）」，第 9 节的覆盖 / 并存对照表补「规则数值：后者覆盖前者」一行；分层图补 `package/基础`、`package/身份场`、`package/标准`，并写清 `package/ → rule → core` 的依赖方向
- [x] 6.2 `moe-kill-dev/SKILL.md` 目录表：`server/core/` 补桌子与玩家；`package/` 行补三个包的名字
- [ ] 6.3 `sanguosha-rules`：§1 身份配置与 §5 距离的口径对齐实现（主公 +1 走改上限、身份写标签、距离由桌子求值）；规则集侧写法补 `rule:setValue` 与 `rule:*` 工厂的用法示例（不再出现 `core.*`）
