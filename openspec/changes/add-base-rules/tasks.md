...
- [x] 3.2 注入面只保留 `rule`（`core` / `require` / `io` / `os` 都拿不到），并在 `rule` 上补四个最小工厂：`createAttributeSystem()` / `createCard(name)` / `createZone()` / `createOrderedZone()`；验证：规则集里能调这四个工厂的用例通过、写 `core.` 会报错的用例通过、既有「拿不到 require」用例继续通过
- [x] 3.3 时机名统一改成 `分类-动作` 风格（`'游戏开始'` → `'游戏-开始'`）：改既有测试与文档里的名字，并确认注册/触发/覆盖语义不变；验证：`--test rule.event` 全绿
- [x] 3.4 事件参数 meta：新增 `server/rule/env-meta.lua`（`---@meta`，不参与运行），声明注入的全局 `rule`、`Rule.EventCtx.<事件名>` 与 `rule:on` / `rule:fire` 的重载签名；验证：问题面板不报 `duplicate-doc-field`，且规则集里 `ctx.xxx` 能按事件名收窄出类型（拿一个探针文件确认）
- [x] 3.5 `server/rule/env-meta.lua` 去掉 `core` 的全局声明；验证：`package/` 下写 `core.` 报未定义全局、问题面板不残留

## 4. 规则包

- [x] 4.1 `package/基础/配置.lua`：用 `rule:setValues` 落默认值（体力上限 4、主公体力上限加成 1 等）；验证：读取到默认值的用例通过
- [x] 4.2 `package/基础/体力.lua`：用 `rule:createAttributeSystem()` 建属性系统，定义 `体力` / `体力上限` 两个属性；验证：属性可用的用例通过，且文件里不出现 `core.`
- [x] 4.3 `package/基础/牌堆.lua`：注册「游戏-开始」，用 `rule:createCard(名字)` / `rule:createOrderedZone()` 按牌表建牌实例 → 放入有序牌区 → 用注入的随机源洗牌；牌表缺失时报错（不写牌堆）；验证：三个用例通过，且文件里不出现 `core.`
...
- [x] 4.7 **牌表逐条核对**：张数按标准版列一遍，交给用户过目（数据错了只改这一处）；验证：核对记录写进本任务说明。**核对记录（用户 2026-09-19：牌表不重要、也不好判断对不对，不阻塞开发 —— 先按草稿放着，等做「杀」/ 判定时再逐条核对）**：基本牌 53（杀 30 / 闪 15 / 桃 8）；锦囊 31（过河拆桥 6 / 顺手牵羊 5 / 无中生有 4 / 无懈可击 4 / 决斗 3 / 南蛮入侵 3 / 五谷丰登 2 / 借刀杀人 2 / 万箭齐发 1 / 桃园结义 1）；装备 18（武器 9：诸葛连弩 2 / 青釭剑 1 / 雌雄双股剑 1 / 青龙偃月刀 1 / 丈八蛇矛 1 / 贯石斧 1 / 方天画戟 1 / 麒麟弓 1；防具 3：八卦阵 2 / 仁王盾 1；+1 马 3：绝影 1 / 的卢 1 / 爪黄飞电 1；−1 马 3：赤兔 1 / 大宛 1 / 紫骍 1）；**合计 102 张**。列成 `rule:setValue('牌表', { { name = '杀', count = 30 }, ... })`

## 5. 装配与测试

- [x] 5.1 `server/test/rule/setup.lua`：8 人完整开局（建属性系统 / Desk / 玩家 / 牌区 → `rule:fire('游戏-开始', ctx)`）断言：每人有身份、体力 = 上限、主公上限比别人多 1、首行动者是主公、牌堆张数等于牌表总张数；**装配方直接用 `moe.core.*`（它不是包，不受注入面限制）**；验证：该用例通过
...
- [x] 6.1 `architecture.md`：第 9 节补「执行环境注入面（只有 `rule`，MUST NOT 注入 `core`）」，第 9 节的覆盖 / 并存对照表补「规则数值：后者覆盖前者」一行；分层图补 `package/基础`、`package/身份场`、`package/标准`，并写清 `package/ → rule → core` 的依赖方向
- [x] 6.2 `moe-kill-dev/SKILL.md` 目录表：`server/core/` 补桌子与玩家；`package/` 行补三个包的名字
- [x] 6.3 `sanguosha-rules`：§1 身份配置与 §5 距离的口径对齐实现（主公 +1 走改上限、身份写标签、距离由桌子求值）；规则集侧写法补 `rule:setValue` 与 `rule:*` 工厂的用法示例（不再出现 `core.*`）
