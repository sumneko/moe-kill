-- 装备（官方通用口径）：装备区有四条槽位（开局设一次）；装备牌的加成写进属性
...
-- 装上（玩家，牌）：按这张牌的数据加修正（牌离开装备区时自动撤销）
---@param player Player
---@param card Card
...
function equip(player, card)
    ...
    if range then
        card:withZone(attrs:addModifier('攻击范围', range))
    end
    local delta = def:getValue('距离修正')
    if delta and def:isKind('进攻马') then
        card:withZone(attrs:addModifier('进攻修正', delta))
    elseif delta then
        card:withZone(attrs:addModifier('防御修正', delta))
    end
end
